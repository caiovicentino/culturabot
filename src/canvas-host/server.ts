...
<title>Culturabuilder Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.culturabuilderCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      window.culturabuilderCanvasA2UIAction &&
        typeof window.culturabuilderCanvasA2UIAction.postMessage === "function"
    );
  const hasHelper = () =>
    typeof window.culturabuilderSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("culturabuilder:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Culturabuilder node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.culturabuilderSendUserAction === "function"
        ? window.culturabuilderSendUserAction
        : window.culturabuilderSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CULTURABUILDER_SKIP_CANVAS_HOST)) return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Culturabuilder Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
