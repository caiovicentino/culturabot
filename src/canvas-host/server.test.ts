...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("culturabuilderCanvasA2UIAction");
    expect(out).toContain("culturabuilderSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("culturabuilderSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__culturabuilder__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("culturabuilder-a2ui-host");
      expect(html).toContain("culturabuilderCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__culturabuilder__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("culturabuilderA2UI");
    } finally {
      await server.close();
      await fs.rm(dir, { recursive: true, force: true });
    }
