...
import { discoverCulturabuilderPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { CulturabuilderPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  culturabuilder?: CulturabuilderPackageManifest;
};
...
const ENV_CATALOG_PATHS = [
  "CULTURABUILDER_PLUGIN_CATALOG_PATHS",
  "CULTURABUILDER_MPM_CATALOG_PATHS",
];
...
function toChannelMeta(params: {
  channel: NonNullable<CulturabuilderPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: CulturabuilderPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageCulturabuilder?: CulturabuilderPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageCulturabuilder;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.culturabuilder;
  return buildCatalogEntry({
    packageName: entry.name,
    packageCulturabuilder: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverCulturabuilderPlugins({ workspaceDir: options.workspaceDir });
