...
function setWhatsAppDmPolicy(cfg: CulturabuilderConfig, dmPolicy: DmPolicy): CulturabuilderConfig {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Culturabuilder can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Culturabuilder" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("culturabuilder channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
