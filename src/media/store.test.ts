...
describe("media store", () => {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.CULTURABUILDER_STATE_DIR = path.join(home, ".culturabuilder");
    ...
    await fs.mkdir(path.join(home, ".culturabuilder"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.culturabuilder${path.sep}media`);
