import type { CulturabuilderConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: CulturabuilderConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: CulturabuilderConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: CulturabuilderConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: CulturabuilderConfig) {
