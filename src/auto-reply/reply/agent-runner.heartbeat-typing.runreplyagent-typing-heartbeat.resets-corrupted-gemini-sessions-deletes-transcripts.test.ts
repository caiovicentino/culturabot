...
describe("runReplyAgent typing (heartbeat)", () => {
  it("resets corrupted Gemini sessions and deletes transcripts", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "culturabuilder-session-reset-"));
  ...
  it("keeps sessions intact on other errors", async () => {
    const prevStateDir = process.env.CULTURABUILDER_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "culturabuilder-session-noreset-"));
    process.env.CULTURABUILDER_STATE_DIR = stateDir;
    try {
