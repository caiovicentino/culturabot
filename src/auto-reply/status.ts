...
type AgentConfig = Partial<NonNullable<NonNullable<CulturabuilderConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.culturabuilder/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `CulturaBuilder ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: CulturabuilderConfig): string {
  const lines = ["ℹ️ Help", ""];
