...
type WebSearchConfig = NonNullable<CulturabuilderConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: CulturabuilderConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("culturabuilder configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.culturabuilder.com/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://culturabuilder.com",
      "X-Title": "Culturabuilder Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
