...
import { __testing, createCulturabuilderCodingTools } from "./pi-tools.js";
...
describe("createCulturabuilderCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createCulturabuilderTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createCulturabuilderCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "culturabuilder-outside.txt");
