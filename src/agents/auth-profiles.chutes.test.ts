...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.CULTURABUILDER_AGENT_DIR;
    else process.env.CULTURABUILDER_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-chutes-"));
    process.env.CULTURABUILDER_STATE_DIR = tempDir;
    process.env.CULTURABUILDER_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
