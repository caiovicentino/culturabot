...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "culturabuilder-models-" });
}

const MODELS_CONFIG: CulturabuilderConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureCulturabuilderModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureCulturabuilderModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureCulturabuilderModelsJson } = await import("./models-config.js");
      const { resolveCulturabuilderAgentDir } = await import("./agent-paths.js");

      await ensureCulturabuilderModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveCulturabuilderAgentDir(), "models.json");
  ...
  it("adds minimax provider when MINIMAX_API_KEY is set", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        await ensureCulturabuilderModelsJson({});
