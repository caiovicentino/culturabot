...
const defaultTools = createCulturabuilderCodingTools();

describe("createCulturabuilderCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createCulturabuilderCodingTools();
    ...
    try {
      ...
      const contents = "Hello from culturabuilder read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "culturabuilder-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "culturabuilder-workspace"),
    ...
    const tools = createCulturabuilderCodingTools({ sandbox });
