...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    args.push("-e", `CULTURABUILDER_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push(
      "-e",
      `CULTURABUILDER_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`,
    );
    args.push("-e", `CULTURABUILDER_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `CULTURABUILDER_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `CULTURABUILDER_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
