...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "culturabuilder.sandbox=1");
  args.push("--label", `culturabuilder.sessionKey=${params.scopeKey}`);
  args.push("--label", `culturabuilder.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `culturabuilder.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "culturabuilder.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`culturabuilder sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`culturabuilder sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("culturabuilder sandbox recreate --all");
}
