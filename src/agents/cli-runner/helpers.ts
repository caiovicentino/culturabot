...
/**
 * Cleanup suspended Culturabuilder CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: CulturabuilderConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-cli-images-"));
  const paths: string[] = [];
