...
import { createCulturabuilderCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("culturabuilder-ws-", async (workspaceDir) => {
      await withTempDir("culturabuilder-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createCulturabuilderCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("culturabuilder-ws-", async (workspaceDir) => {
      await withTempDir("culturabuilder-override-", async (overrideDir) => {
        const tools = createCulturabuilderCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("culturabuilder-sandbox-", async (sandboxDir) => {
      await withTempDir("culturabuilder-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "culturabuilder-sbx-test",
        ...
        const tools = createCulturabuilderCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
