...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "culturabuilder-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".culturabuilder", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<CulturabuilderConfig> = {},
): CulturabuilderConfig {
  const base: CulturabuilderConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".culturabuilder", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
