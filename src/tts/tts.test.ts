...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    it("skips auto-TTS when inbound audio gating is on and the message is not audio", async () => {
      const prevPrefs = process.env.CULTURABUILDER_TTS_PREFS;
      process.env.CULTURABUILDER_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      process.env.CULTURABUILDER_TTS_PREFS = prevPrefs;
    });
