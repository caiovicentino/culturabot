...
import type { CulturabuilderConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: CulturabuilderConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep CULTURABUILDER_STATE_DIR on a local-only volume and re-run "${formatCliCommand("culturabuilder security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: CulturabuilderConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer CULTURABUILDER_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: CulturabuilderConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: CulturabuilderConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: CulturabuilderConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: CulturabuilderConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: CulturabuilderConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: CulturabuilderConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: CulturabuilderConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: CulturabuilderConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function collectExposureMatrixFindings(cfg: CulturabuilderConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
