  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Culturabuilder state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Culturabuilder state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectBrowserControlFindings(cfg: CulturabuilderConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("culturabuilder security audit --deep")}".`,
    });
    return findings;
  }
...
function collectLoggingFindings(cfg: CulturabuilderConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: CulturabuilderConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
    ...
    const token =
      mode === "remote"
        ? typeof remote?.token === "string" && remote.token.trim()
          ? remote.token.trim()
          : undefined
        : process.env.CULTURABUILDER_GATEWAY_TOKEN?.trim() ||
          (typeof authToken === "string" && authToken.trim() ? authToken.trim() : undefined);
    const password =
      process.env.CULTURABUILDER_GATEWAY_PASSWORD?.trim() ||
      (mode === "remote"
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && deep.gateway.ok === false) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("culturabuilder status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("culturabuilder security audit --deep")}".`,
    });
  }
