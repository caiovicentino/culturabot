...
export function listSlackAccountIds(cfg: CulturabuilderConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: CulturabuilderConfig): string {
...
function mergeSlackAccountConfig(cfg: CulturabuilderConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: CulturabuilderConfig): ResolvedSlackAccount[] {
