...
export function listSignalAccountIds(cfg: CulturabuilderConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: CulturabuilderConfig): string {
...
export function listEnabledSignalAccounts(cfg: CulturabuilderConfig): ResolvedSignalAccount[] {
