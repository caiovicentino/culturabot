...
type PackageManifest = {
  ...
  culturabuilder?: { extensions?: string[] };
};
...
async function ensureCulturabuilderExtensions(manifest: PackageManifest) {
  const extensions = manifest.culturabuilder?.extensions;
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing culturabuilder.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json culturabuilder.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureCulturabuilderExtensions(manifest);
  } catch (err) {
...
export async function installPluginFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-plugin-"));
...
export async function installPluginFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-npm-pack-"));
