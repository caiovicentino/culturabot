...
import { discoverCulturabuilderPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: CulturabuilderPluginDefinition;
  register?: CulturabuilderPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as CulturabuilderPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as CulturabuilderPluginDefinition;
...
export function loadCulturabuilderPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverCulturabuilderPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "culturabuilder/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: CulturabuilderPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as CulturabuilderPluginModule;
    } catch (err) {
