...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `culturabuilder-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.culturabuilder/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        culturabuilder: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without culturabuilder.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@culturabuilder/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    if (result.ok) return;
    expect(result.error).toContain("culturabuilder.extensions");
  });
});
