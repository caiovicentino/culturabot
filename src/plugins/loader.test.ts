...
import { loadCulturabuilderPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.CULTURABUILDER_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `culturabuilder-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "culturabuilder.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.CULTURABUILDER_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadCulturabuilderPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.CULTURABUILDER_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadCulturabuilderPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadCulturabuilderPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@culturabuilder/memory-core",
        version: "1.2.3",
