...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.CULTURABUILDER_LAUNCHD_LABEL?.trim();
  if (envLabel) return envLabel;
  return resolveGatewayLaunchAgentLabel(args?.env?.CULTURABUILDER_PROFILE);
}
...
export function resolveGatewayLogPaths(env: Record<string, string | undefined>): {
  ...
  const prefix = env.CULTURABUILDER_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
export async function findLegacyLaunchAgents(
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.CULTURABUILDER_PROFILE)) {
...
export async function installLaunchAgent({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.CULTURABUILDER_PROFILE)) {
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: env.CULTURABUILDER_PROFILE,
      version: environment?.CULTURABUILDER_SERVICE_VERSION ?? env.CULTURABUILDER_SERVICE_VERSION,
    });
