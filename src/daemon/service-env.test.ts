...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.CULTURABUILDER_GATEWAY_PORT).toBe("18789");
    expect(env.CULTURABUILDER_GATEWAY_TOKEN).toBe("secret");
    expect(env.CULTURABUILDER_SERVICE_MARKER).toBe("culturabuilder");
    expect(env.CULTURABUILDER_SERVICE_KIND).toBe("gateway");
    expect(typeof env.CULTURABUILDER_SERVICE_VERSION).toBe("string");
    expect(env.CULTURABUILDER_SYSTEMD_UNIT).toBe("culturabuilder-gateway.service");
    if (process.platform === "darwin") {
      expect(env.CULTURABUILDER_LAUNCHD_LABEL).toBe("com.culturabuilder.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", CULTURABUILDER_PROFILE: "work" },
      port: 18789,
    });
    expect(env.CULTURABUILDER_SYSTEMD_UNIT).toBe("culturabuilder-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.CULTURABUILDER_LAUNCHD_LABEL).toBe("com.culturabuilder.work");
    }
  });
});
