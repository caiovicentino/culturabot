...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("culturabuilder node install"),
    formatCliCommand("culturabuilder node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(
        `Reinstall with: ${formatCliCommand("culturabuilder node install --force")}`,
      );
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    CULTURABUILDER_LOG_PREFIX: baseEnv.CULTURABUILDER_LOG_PREFIX ?? "node",
