...
const SAFE_DAEMON_ENV_KEYS = [
  "CULTURABUILDER_STATE_DIR",
  "CULTURABUILDER_CONFIG_PATH",
  "CULTURABUILDER_PROFILE",
  "CULTURABUILDER_STATE_DIR",
  "CULTURABUILDER_CONFIG_PATH",
  "CULTURABUILDER_GATEWAY_PORT",
  "CULTURABUILDER_NIX_MODE",
];
...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(
      `Service not installed. Run: ${formatCliCommand("culturabuilder gateway install", env)}`,
    );
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.CULTURABUILDER_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("culturabuilder gateway install", env),
    formatCliCommand("culturabuilder gateway", env),
  ];
