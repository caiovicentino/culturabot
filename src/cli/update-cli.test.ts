...
vi.mock("../infra/culturabuilder-root.js", () => ({
  resolveCulturabuilderPackageRoot: vi.fn(),
}));
...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveCulturabuilderPackageRoot } = await import("../infra/culturabuilder-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveCulturabuilderPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Culturabuilder update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveCulturabuilderPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-update-wizard-"));
    const previousGitDir = process.env.CULTURABUILDER_GIT_DIR;
    try {
      setTty(true);
      process.env.CULTURABUILDER_GIT_DIR = tempDir;
      ...
      process.env.CULTURABUILDER_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
