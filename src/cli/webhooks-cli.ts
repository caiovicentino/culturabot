...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.culturabuilder.com/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Culturabuilder hooks")
    ...
    .option("--hook-url <url>", "Culturabuilder hook URL")
    .option("--hook-token <token>", "Culturabuilder hook token")
