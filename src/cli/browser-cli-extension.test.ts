...
describe("browser extension install", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "culturabuilder-ext-"));
  ...
  it("copies extension path to clipboard", async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "culturabuilder-ext-path-"));
    ...
    try {
      ...
      if (prev === undefined) delete process.env.CULTURABUILDER_STATE_DIR;
      else process.env.CULTURABUILDER_STATE_DIR = prev;
    }
  });
});
