...
const ETC_CULTURABUILDER_DIR = path.join(ROOT_DIR, "etc", "culturabuilder");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "culturabuilder.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcCulturabuilderPath(...parts: string[]) {
  return path.join(ETC_CULTURABUILDER_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcCulturabuilderPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "culturabuilder.json"))).toEqual({
      shared: true,
    });
  });
});
