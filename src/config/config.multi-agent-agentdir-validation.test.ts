...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    ...
    const shared = path.join(tmpdir(), "culturabuilder-shared-agentdir");
        ...
        JSON.stringify(
          {
            agents: {
              list: [
                { id: "a", agentDir: "~/.culturabuilder/agents/shared/agent" },
                { id: "b", agentDir: "~/.culturabuilder/agents/shared/agent" },
              ],
            },
