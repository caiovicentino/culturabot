...
describe("sessions", () => {
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-sessions-"));
  ...
  it("derives session transcripts dir from CULTURABUILDER_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CULTURABUILDER_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.CULTURABUILDER_STATE_DIR = "/custom/state";
    try {
