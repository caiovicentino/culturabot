...
import type { CulturabuilderConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareCulturabuilderVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "CULTURABUILDER_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): CulturabuilderConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as CulturabuilderConfig;
}
...
function stampConfigVersion(cfg: CulturabuilderConfig): CulturabuilderConfig {
...
function warnIfConfigFromFuture(
  ...
  if (!touched) return;
  const cmp = compareCulturabuilderVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Culturabuilder (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: CulturabuilderConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): CulturabuilderConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as CulturabuilderConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        resolvedConfig as CulturabuilderConfig,
        {
          env: deps.env,
  ...
  async function writeConfigFile(cfg: CulturabuilderConfig) {
    clearConfigCache();
...
// module scope. `CULTURABUILDER_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CULTURABUILDER_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.CULTURABUILDER_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): CulturabuilderConfig {
...
export async function writeConfigFile(cfg: CulturabuilderConfig): Promise<void> {
  clearConfigCache();
