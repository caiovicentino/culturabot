  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.culturabuilder/logs/culturabuilder.log" },
        hooks: {
          path: "~/.culturabuilder/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.culturabuilder/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(
        path.join(home, ".culturabuilder", "logs", "culturabuilder.log"),
      );
      expect(cfg.hooks?.path).toBe(path.join(home, ".culturabuilder", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".culturabuilder", "telegram.token"),
      );
