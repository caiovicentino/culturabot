...
import type { CulturabuilderConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: CulturabuilderConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: CulturabuilderConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = CulturabuilderSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as CulturabuilderConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as CulturabuilderConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as CulturabuilderConfig)),
    ),
  };
}
