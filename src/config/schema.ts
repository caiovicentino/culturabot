...
import { CulturabuilderSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof CulturabuilderSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when Culturabuilder writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /culturabuilder).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $CULTURABUILDER_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.culturabuilder/memory/{agentId}.sqlite).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `culturabuilder plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.culturabuilder/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/culturabuilder",
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = CulturabuilderSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "CulturabuilderConfig";
