...
function listConfiguredAccountIds(cfg: CulturabuilderConfig): string[] {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts || typeof accounts !== "object") return [];
  return Object.keys(accounts).filter(Boolean);
}

export function listWhatsAppAuthDirs(cfg: CulturabuilderConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: CulturabuilderConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: CulturabuilderConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: CulturabuilderConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: CulturabuilderConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: CulturabuilderConfig): ResolvedWhatsAppAccount[] {
