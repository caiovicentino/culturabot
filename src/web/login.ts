...
export async function loginWeb(
  ...
  try {
    ...
    if (code === DisconnectReason.loggedOut) {
      ...
      console.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("culturabuilder channels login")} and scan the QR again.`,
        ),
      );
