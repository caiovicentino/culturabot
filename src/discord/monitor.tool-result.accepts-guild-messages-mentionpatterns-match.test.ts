...
vi.mock("../config/sessions.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveStorePath: vi.fn(() => "/tmp/culturabuilder-sessions.json"),
...
describe("discord tool result dispatch", () => {
  it("accepts guild messages when mentionPatterns match", async () => {
    ...
    const cfg = {
      ...
      session: { store: "/tmp/culturabuilder-sessions.json" },
      channels: {
        discord: {
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Culturabuilder" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
