    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "culturabuilder", messages: [{ role: "user", content: "hi" }] },
          { "x-culturabuilder-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-culturabuilder-agent-id": "alpha" },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-culturabuilder-agent-id": "beta",
            "x-culturabuilder-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
