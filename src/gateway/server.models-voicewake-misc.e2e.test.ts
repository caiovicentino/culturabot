...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.CULTURABUILDER_STATE_DIR = path.join(homeDir, ".culturabuilder");
    ...
    return () => {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.CULTURABUILDER_STATE_DIR = prevStateDir;
      }
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-home-"));
      ...
      const onDisk = JSON.parse(
        await fs.readFile(
          path.join(homeDir, ".culturabuilder", "settings", "voicewake.json"),
          "utf8",
        ),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.CULTURABUILDER_CANVAS_HOST_PORT;
    process.env.CULTURABUILDER_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.CULTURABUILDER_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevToken === undefined) {
      ...
      process.env.CULTURABUILDER_GATEWAY_TOKEN = prevToken;
    }
    if (prevCanvasPort === undefined) {
      delete process.env.CULTURABUILDER_CANVAS_HOST_PORT;
    } else {
      process.env.CULTURABUILDER_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.CULTURABUILDER_CONFIG_PATH;
    if (!configPath) throw new Error("Missing CULTURABUILDER_CONFIG_PATH");
