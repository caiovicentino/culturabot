...
export function resolveHooksConfig(cfg: CulturabuilderConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-culturabuilder-token"] === "string"
      ? req.headers["x-culturabuilder-token"].trim()
      : "";
