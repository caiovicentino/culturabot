    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.CULTURABUILDER_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info(
          "skipping gmail watcher restart (CULTURABUILDER_SKIP_GMAIL_WATCHER=1)",
        );
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (CULTURABUILDER_SKIP_CHANNELS=1 or CULTURABUILDER_SKIP_PROVIDERS=1)",
        );
      } else {
