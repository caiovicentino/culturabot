...
import { createCulturabuilderTools } from "../agents/culturabuilder-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.CULTURABUILDER_GATEWAY_PORT;
  prevGatewayToken = process.env.CULTURABUILDER_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.CULTURABUILDER_GATEWAY_PORT = String(gatewayPort);
  process.env.CULTURABUILDER_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.CULTURABUILDER_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.CULTURABUILDER_GATEWAY_TOKEN;
  } else {
    process.env.CULTURABUILDER_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createCulturabuilderTools().find(
