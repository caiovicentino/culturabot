...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.CULTURABUILDER_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-gw-mock-home-"));
      ...
      const toolProbePath = path.join(workspaceDir, `.culturabuilder-tool-probe.${nonceA}.txt`);
      ...
      try {
        ...
        process.env.CULTURABUILDER_CONFIG_PATH = prev.configPath;
        process.env.CULTURABUILDER_GATEWAY_TOKEN = prev.token;
        process.env.CULTURABUILDER_SKIP_CHANNELS = prev.skipChannels;
        process.env.CULTURABUILDER_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.CULTURABUILDER_SKIP_CRON = prev.skipCron;
        process.env.CULTURABUILDER_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.CULTURABUILDER_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-wizard-home-"));
    process.env.HOME = tempHome;
    delete process.env.CULTURABUILDER_STATE_DIR;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.CULTURABUILDER_STATE_DIR = prev.stateDir;
      process.env.CULTURABUILDER_CONFIG_PATH = prev.configPath;
      process.env.CULTURABUILDER_GATEWAY_TOKEN = prev.token;
      process.env.CULTURABUILDER_SKIP_CHANNELS = prev.skipChannels;
      process.env.CULTURABUILDER_SKIP_GMAIL_WATCHER = prev.skipGmail;
