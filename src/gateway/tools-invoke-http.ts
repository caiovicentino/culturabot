...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-culturabuilder-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-culturabuilder-account-id")?.trim() || undefined;
  ...
  const allTools = createCulturabuilderTools({
