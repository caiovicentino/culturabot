 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for Culturabuilder Gateway.
 *
      ...
      const content =
        Array.isArray(payloads) && payloads.length > 0
          ? payloads
              ...
              .filter(Boolean)
              .join("\n\n")
          : "No response from Culturabuilder.";
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText = accumulatedText || "No response from Culturabuilder.";
