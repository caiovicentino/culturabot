...
import { resolveCulturabuilderAgentDir } from "../agents/agent-paths.js";
...
import { ensureCulturabuilderModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { CulturabuilderConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.CULTURABUILDER_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.CULTURABUILDER_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.CULTURABUILDER_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: CulturabuilderConfig;
  agentDir: string;
}): CulturabuilderConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<CulturabuilderConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.CULTURABUILDER_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.CULTURABUILDER_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveCulturabuilderAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-live-state-"));
  process.env.CULTURABUILDER_STATE_DIR = tempStateDir;
  ...
  process.env.CULTURABUILDER_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.culturabuilder-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveCulturabuilderAgentDir();
  const sanitizedCfg: CulturabuilderConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-live-"));
  ...
  await ensureCulturabuilderModelsJson(nextCfg);
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Culturabuilder live tool probe (local, safe): " +
    ...
    process.env.CULTURABUILDER_CONFIG_PATH = previous.configPath;
    process.env.CULTURABUILDER_GATEWAY_TOKEN = previous.token;
    process.env.CULTURABUILDER_SKIP_CHANNELS = previous.skipChannels;
    process.env.CULTURABUILDER_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.CULTURABUILDER_SKIP_CRON = previous.skipCron;
    process.env.CULTURABUILDER_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.CULTURABUILDER_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.CULTURABUILDER_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureCulturabuilderModelsJson(cfg);
      ...
      const rawModels = process.env.CULTURABUILDER_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const toolProbePath = path.join(
      workspaceDir,
      `.culturabuilder-live-zai-fallback.${nonceA}.txt`,
    );
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
