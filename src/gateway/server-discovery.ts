...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Culturabuilder";
  if (/culturabuilder/i.test(trimmed)) return trimmed;
  return `${trimmed} (Culturabuilder)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.CULTURABUILDER_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "culturabuilder");
  ...
  const binCli = path.join(cwd, "bin", "culturabuilder.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.CULTURABUILDER_TAILNET_DNS?.trim();
