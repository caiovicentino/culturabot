...
beforeAll(async () => {
  previousToken = process.env.CULTURABUILDER_GATEWAY_TOKEN;
...
afterAll(async () => {
  await server.close();
  if (previousToken === undefined) delete process.env.CULTURABUILDER_GATEWAY_TOKEN;
  else process.env.CULTURABUILDER_GATEWAY_TOKEN = previousToken;
});
...
describe("gateway server sessions", () => {
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
