...
declare const __CULTURABUILDER_VERSION__: string;
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `culturabuilder --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.CULTURABUILDER_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.CULTURABUILDER_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
    bindRaw === "loopback" ||
...
void main().catch((err) => {
  console.error(
    "[culturabuilder] Gateway daemon failed:",
