...
const execHostEnforced = process.env.CULTURABUILDER_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.CULTURABUILDER_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureCulturabuilderCliOnPath({ pathEnv: process.env.PATH ?? "" });
