...
export async function handlePortError(
  ...
  if (err instanceof PortInUseError || (isErrno(err) && err.code === "EADDRINUSE")) {
    ...
    if (details) {
      ...
      if (/culturabuilder|src\/index\.ts|dist\/index\.js/.test(details)) {
        runtime.error(
          warn(
            "It looks like another culturabuilder instance is already running. Stop it or pick a different port.",
          ),
        );
      }
    }
