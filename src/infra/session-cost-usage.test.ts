...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-cost-"));
    ...
    const originalState = process.env.CULTURABUILDER_STATE_DIR;
    process.env.CULTURABUILDER_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) delete process.env.CULTURABUILDER_STATE_DIR;
      else process.env.CULTURABUILDER_STATE_DIR = originalState;
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-cost-session-"));
