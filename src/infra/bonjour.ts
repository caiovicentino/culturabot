...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CULTURABUILDER_DISABLE_BONJOUR)) return true;
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Culturabuilder";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Culturabuilder\)\s*$/i, "").trim() || normalized;
}
