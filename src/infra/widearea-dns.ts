...
export const WIDE_AREA_DISCOVERY_DOMAIN = "culturabuilder.internal.";
export const WIDE_AREA_ZONE_FILENAME = "culturabuilder.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*culturabuilder-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "culturabuilder";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "culturabuilder");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "culturabuilder-gw");
  ...
  records.push(`_culturabuilder-gw._tcp IN PTR ${instanceLabel}._culturabuilder-gw._tcp`);
  records.push(
    `${instanceLabel}._culturabuilder-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`,
  );
  records.push(`${instanceLabel}._culturabuilder-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; culturabuilder-content-hash: ${contentHash}\n${contentBody}`;
}
