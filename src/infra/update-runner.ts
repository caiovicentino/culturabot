...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && !pkgRoot) {
    return {
      ...
      mode: "unknown",
      root: gitRoot,
      reason: "not-culturabuilder-root",
      steps: [],
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "culturabuilder-update-preflight-"),
      );
    ...
    const doctorStep = await runStep(
      step(
        ...
        managerScriptArgs(manager, "culturabuilder", ["doctor", "--non-interactive"]),
        gitRoot,
        { CULTURABUILDER_UPDATE_IN_PROGRESS: "1" },
      ),
    );
