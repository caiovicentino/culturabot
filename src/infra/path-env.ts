...
type EnsureCulturabuilderPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureCulturabuilderPathOpts): string[] {
  ...
  // Bundled macOS app: `culturabuilder` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCulturabuilder = path.join(execDir, "culturabuilder");
    if (isExecutable(siblingCulturabuilder)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/culturabuilder` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "culturabuilder"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `culturabuilder` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureCulturabuilderCliOnPath(opts: EnsureCulturabuilderPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CULTURABUILDER_PATH_BOOTSTRAPPED)) return;
  process.env.CULTURABUILDER_PATH_BOOTSTRAPPED = "1";
