...
export function triggerCulturabuilderRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.CULTURABUILDER_SYSTEMD_UNIT,
        process.env.CULTURABUILDER_PROFILE,
      );
  ...
  const label =
    process.env.CULTURABUILDER_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CULTURABUILDER_PROFILE);
