...
function resolveActiveHoursTimezone(cfg: CulturabuilderConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: CulturabuilderConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: CulturabuilderConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: CulturabuilderConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: CulturabuilderConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: CulturabuilderConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: CulturabuilderConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: CulturabuilderConfig) => {
    if (state.stopped) return;
    const now = Date.now();
