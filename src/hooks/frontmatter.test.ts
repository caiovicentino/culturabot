...
`;
    ...
    expect(parsed.culturabuilder.emoji).toBe("💾");
    expect(parsed.culturabuilder.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.culturabuilder.emoji).toBe("📝");
    expect(parsed.culturabuilder.events).toEqual(["command"]);
    expect(parsed.culturabuilder.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.culturabuilder.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"culturabuilder": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"culturabuilder": {"events": ["test"]}}');
  });
...
describe("resolveCulturabuilderMetadata", () => {
  it("extracts culturabuilder metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveCulturabuilderMetadata(frontmatter);
  ...
  it("returns undefined when culturabuilder key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        culturabuilder: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Culturabuilder" },
            { id: "npm", kind: "npm", package: "@culturabuilder/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@culturabuilder/hook");
  });
...
`;
    ...
    const culturabuilder = resolveCulturabuilderMetadata(frontmatter);
    expect(culturabuilder).toBeDefined();
    expect(culturabuilder?.emoji).toBe("💾");
    expect(culturabuilder?.events).toEqual(["command:new"]);
    expect(culturabuilder?.requires?.config).toEqual(["workspace.dir"]);
    expect(culturabuilder?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const culturabuilder = resolveCulturabuilderMetadata(frontmatter);
    expect(culturabuilder?.emoji).toBe("disk");
    expect(culturabuilder?.events).toEqual(["command:new"]);
  });
});
