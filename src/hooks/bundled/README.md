...
This directory contains hooks that ship with Culturabuilder. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.culturabuilder.com/hooks/soul-evil
...
metadata:
  { "culturabuilder": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.culturabuilder/culturabuilder.json`:
...
2. Restart gateway: `pkill -9 -f 'culturabuilder.*gateway' && pnpm culturabuilder gateway`
3. Enable the hook: `culturabuilder hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.culturabuilder.com/hooks
