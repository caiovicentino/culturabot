...
homepage: https://docs.culturabuilder.com/hooks#command-logger
metadata:
  {
    "culturabuilder":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.culturabuilder/logs/commands.log`
...
`~/.culturabuilder/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.culturabuilder/logs/commands.log ~/.culturabuilder/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/culturabuilder`:
   ```
   /home/username/.culturabuilder/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
