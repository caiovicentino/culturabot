...
async function ensureCulturabuilderHooks(manifest: HookPackageManifest) {
  const hooks = manifest.culturabuilder?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing culturabuilder.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json culturabuilder.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureCulturabuilderHooks(manifest);
  } catch (err) {
...
export async function installHooksFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-hook-"));
...
export async function installHooksFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-hook-pack-"));
