...
import type { CulturabuilderPluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: CulturabuilderPluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: CulturabuilderPluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
