...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.CULTURABUILDER_STATE_DIR;
    prevBundledDir = process.env.CULTURABUILDER_BUNDLED_HOOKS_DIR;
    process.env.CULTURABUILDER_STATE_DIR = path.join(baseDir, "state");
    process.env.CULTURABUILDER_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.CULTURABUILDER_BUNDLED_HOOKS_DIR;
    } else {
      process.env.CULTURABUILDER_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          culturabuilder: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"culturabuilder":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
