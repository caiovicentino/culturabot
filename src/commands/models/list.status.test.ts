...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveCulturabuilderAgentDir: vi.fn().mockReturnValue("/tmp/culturabuilder-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveCulturabuilderAgentDir: mocks.resolveCulturabuilderAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/culturabuilder-agent/auth-profiles.json");
