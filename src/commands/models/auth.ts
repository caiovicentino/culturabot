...
import { readConfigFileSnapshot, type CulturabuilderConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: CulturabuilderConfig, model: string): CulturabuilderConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("culturabuilder plugins install")}\`.`,
    );
  }
