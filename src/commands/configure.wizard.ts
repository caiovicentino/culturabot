    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from culturabuilder.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Culturabuilder update wizard" : "Culturabuilder configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: CulturabuilderConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.culturabuilder.com/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("culturabuilder doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.CULTURABUILDER_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.CULTURABUILDER_GATEWAY_PASSWORD,
    });
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.CULTURABUILDER_GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env.CULTURABUILDER_GATEWAY_PASSWORD;
    ...
    const newPassword =
      nextConfig.gateway?.auth?.password ?? process.env.CULTURABUILDER_GATEWAY_PASSWORD;
    const oldPassword =
      baseConfig.gateway?.auth?.password ?? process.env.CULTURABUILDER_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.CULTURABUILDER_GATEWAY_TOKEN;
