...
export function listAgentEntries(cfg: CulturabuilderConfig): AgentEntry[] {
...
function resolveAgentName(cfg: CulturabuilderConfig, agentId: string) {
...
function resolveAgentModel(cfg: CulturabuilderConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: CulturabuilderConfig): AgentSummary[] {
