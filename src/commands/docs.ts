...
const SEARCH_TOOL = "https://docs.culturabuilder.com/mcp.SearchCulturabuilder";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.culturabuilder.com");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(
        `${theme.muted("Search:")} ${formatCliCommand('culturabuilder docs "your query"')}`,
      );
    } else {
      runtime.log("Docs: https://docs.culturabuilder.com/");
      runtime.log(`Search: ${formatCliCommand('culturabuilder docs "your query"')}`);
    }
    return;
  }
