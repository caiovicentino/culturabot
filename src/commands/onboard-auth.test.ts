...
const requireAgentDir = () => {
  const agentDir = process.env.CULTURABUILDER_AGENT_DIR;
  if (!agentDir) throw new Error("CULTURABUILDER_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.CULTURABUILDER_STATE_DIR;
  const previousAgentDir = process.env.CULTURABUILDER_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.CULTURABUILDER_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.CULTURABUILDER_AGENT_DIR;
    } else {
      process.env.CULTURABUILDER_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.CULTURABUILDER_OAUTH_DIR;
  });

  it("writes auth-profiles.json under CULTURABUILDER_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-oauth-"));
    process.env.CULTURABUILDER_STATE_DIR = tempStateDir;
    process.env.CULTURABUILDER_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.CULTURABUILDER_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to CULTURABUILDER_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-minimax-"));
    process.env.CULTURABUILDER_STATE_DIR = tempStateDir;
    process.env.CULTURABUILDER_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.CULTURABUILDER_AGENT_DIR;
