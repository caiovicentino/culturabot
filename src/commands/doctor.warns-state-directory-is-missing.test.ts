...
beforeEach(() => {
  ...
  resolveCulturabuilderPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.CULTURABUILDER_STATE_DIR;
  originalUpdateInProgress = process.env.CULTURABUILDER_UPDATE_IN_PROGRESS;
  process.env.CULTURABUILDER_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "culturabuilder-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.CULTURABUILDER_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.CULTURABUILDER_UPDATE_IN_PROGRESS;
  } else {
    process.env.CULTURABUILDER_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveCulturabuilderPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadCulturabuilderPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/culturabuilder.json",
    createConfigIO,
...
vi.mock("../infra/culturabuilder-root.js", () => ({
  resolveCulturabuilderPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "culturabuilder-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.CULTURABUILDER_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when CULTURABUILDER_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/culturabuilder.json",
      exists: true,
      raw: "{}",
    ...
    process.env.CULTURABUILDER_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
