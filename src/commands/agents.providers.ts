...
function resolveDefaultAccountId(cfg: CulturabuilderConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: CulturabuilderConfig): boolean {
...
export function summarizeBindings(cfg: CulturabuilderConfig, bindings: AgentBinding[]): string[] {
