...
function resolveConfiguredModelRaw(cfg: CulturabuilderConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: CulturabuilderConfig): string[] {
...
export function applyPrimaryModel(cfg: CulturabuilderConfig, model: string): CulturabuilderConfig {
