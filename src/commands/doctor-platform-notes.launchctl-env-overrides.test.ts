...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "CULTURABUILDER_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CULTURABUILDER_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv CULTURABUILDER_GATEWAY_TOKEN");
    expect(message).not.toContain("CULTURABUILDER_GATEWAY_PASSWORD");
  });
