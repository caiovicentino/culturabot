...
import { type CulturabuilderConfig, CONFIG_PATH, writeConfigFile } from "../config/config.js";
...
async function readConfigFileRaw(): Promise<{
  exists: boolean;
  parsed: CulturabuilderConfig;
}> {
  try {
    const raw = await fs.readFile(CONFIG_PATH, "utf-8");
    const parsed = JSON5.parse(raw);
    if (parsed && typeof parsed === "object") {
      return { exists: true, parsed: parsed as CulturabuilderConfig };
    }
