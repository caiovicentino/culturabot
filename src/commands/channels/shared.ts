...
import { type CulturabuilderConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export async function requireValidConfig(
...
): Promise<CulturabuilderConfig | null> {
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("culturabuilder doctor")}.`);
    runtime.exit(1);
    return null;
  }
