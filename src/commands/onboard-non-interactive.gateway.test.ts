...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.CULTURABUILDER_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.CULTURABUILDER_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.CULTURABUILDER_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.CULTURABUILDER_STATE_DIR = stateDir;
    process.env.CULTURABUILDER_CONFIG_PATH = path.join(stateDir, "culturabuilder.json");
