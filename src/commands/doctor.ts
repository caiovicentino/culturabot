...
function resolveMode(cfg: CulturabuilderConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Culturabuilder doctor");

  const root = await resolveCulturabuilderPackageRoot({
  ...
  let cfg: CulturabuilderConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("culturabuilder configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("culturabuilder config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("culturabuilder setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("culturabuilder doctor --fix")}" to apply changes.`);
  }
