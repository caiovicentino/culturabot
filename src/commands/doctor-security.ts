...
import type { CulturabuilderConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: CulturabuilderConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("culturabuilder security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("culturabuilder configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("culturabuilder config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("culturabuilder doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("culturabuilder config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
