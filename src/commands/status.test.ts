...
beforeAll(() => {
  previousProfile = process.env.CULTURABUILDER_PROFILE;
  process.env.CULTURABUILDER_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.CULTURABUILDER_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/culturabuilder-root.js", () => ({
  resolveCulturabuilderPackageRoot: vi.fn().mockResolvedValue("/tmp/culturabuilder"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/culturabuilder/pnpm-lock.yaml",
      markerPath: "/tmp/culturabuilder/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.culturabuilder.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.culturabuilder.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Culturabuilder status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("culturabuilder status --all") ||
          l.includes("culturabuilder --profile isolated status --all") ||
          l.includes("culturabuilder status --all") ||
          l.includes("culturabuilder --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.CULTURABUILDER_GATEWAY_TOKEN = "abcd1234";
    try {
