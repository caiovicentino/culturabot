...
- Replaces the legacy `gateway` command. CLI entry point: `culturabuilder gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
culturabuilder gateway --port 18789
# for full debug/trace logs in stdio:
culturabuilder gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
culturabuilder gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.culturabuilder/culturabuilder.json` (or `CULTURABUILDER_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__culturabuilder__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `CULTURABUILDER_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `CULTURABUILDER_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `CULTURABUILDER_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.culturabuilder.<profile>`
- Linux: `culturabuilder-gateway-<profile>.service`
- Windows: `Culturabuilder Gateway (<profile>)`

Install metadata is embedded in the service config:
- `CULTURABUILDER_SERVICE_MARKER=culturabuilder`
- `CULTURABUILDER_SERVICE_KIND=gateway`
- `CULTURABUILDER_SERVICE_VERSION=<version>`
...
```bash
culturabuilder --dev setup
culturabuilder --dev gateway --allow-unconfigured
# then target the dev instance:
culturabuilder --dev status
culturabuilder --dev health
```

Defaults (can be overridden via env/flags/config):
- `CULTURABUILDER_STATE_DIR=~/.culturabuilder-dev`
- `CULTURABUILDER_CONFIG_PATH=~/.culturabuilder-dev/culturabuilder.json`
- `CULTURABUILDER_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- Base port = `gateway.port` (or `CULTURABUILDER_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `CULTURABUILDER_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `CULTURABUILDER_CONFIG_PATH`
- unique `CULTURABUILDER_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
culturabuilder --profile main gateway install
culturabuilder --profile rescue gateway install
```

Example:
```bash
CULTURABUILDER_CONFIG_PATH=~/.culturabuilder/a.json CULTURABUILDER_STATE_DIR=~/.culturabuilder-a culturabuilder gateway --port 19001
CULTURABUILDER_CONFIG_PATH=~/.culturabuilder/b.json CULTURABUILDER_STATE_DIR=~/.culturabuilder-b culturabuilder gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `culturabuilder health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `culturabuilder`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `culturabuilder gateway install` writes `~/Library/LaunchAgents/com.culturabuilder.gateway.plist`
    (or `com.culturabuilder.<profile>.plist`).
  - `culturabuilder doctor` audits the LaunchAgent config and can update it to current defaults.
...
culturabuilder gateway install
...
- If other gateway-like services are detected, the CLI warns unless they are Culturabuilder profile services.
  ...
  - Cleanup: `culturabuilder gateway uninstall` (current service) and `culturabuilder doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `culturabuilder gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Culturabuilder.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.culturabuilder.gateway` (or `com.culturabuilder.<profile>`).
- To stop it cleanly, use `culturabuilder gateway stop` (or `launchctl bootout gui/$UID/com.culturabuilder.gateway`).
- To restart, use `culturabuilder gateway restart` (or `launchctl kickstart -k gui/$UID/com.culturabuilder.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `culturabuilder gateway install` first.
  - Replace the label with `com.culturabuilder.<profile>` when running a named profile.

## Supervision (systemd user unit)
Culturabuilder installs a **systemd user service** by default on Linux/WSL2. We
...
`culturabuilder gateway install` writes the user unit. `culturabuilder doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/culturabuilder-gateway[-<profile>].service`:
```
[Unit]
Description=Culturabuilder Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/culturabuilder gateway --port 18789
Restart=always
RestartSec=5
Environment=CULTURABUILDER_GATEWAY_TOKEN=
...
```
systemctl --user enable --now culturabuilder-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/culturabuilder-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now culturabuilder-gateway[-<profile>].service
```
...
## CLI helpers
- `culturabuilder gateway health|status` — request health/status over the Gateway WS.
- `culturabuilder message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `culturabuilder agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `culturabuilder gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `culturabuilder gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `culturabuilder gateway` and the legacy TCP control port.
