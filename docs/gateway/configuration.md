---
summary: "All configuration options for ~/.culturabuilder/culturabuilder.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Culturabuilder reads an optional **JSON5** config from `~/.culturabuilder/culturabuilder.json` (comments + trailing commas allowed).

If the file is missing, Culturabuilder uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
Culturabuilder only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `culturabuilder doctor`, `culturabuilder logs`, `culturabuilder health`, `culturabuilder status`, `culturabuilder service`, `culturabuilder help`).
- Run `culturabuilder doctor` to see the exact issues.
- Run `culturabuilder doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `culturabuilder config set`. Keep a backup of `~/.culturabuilder/culturabuilder.json`.
...
```bash
culturabuilder gateway call config.get --params '{}' # capture payload.hash
culturabuilder gateway call config.apply --params '{
...
```bash
culturabuilder gateway call config.get --params '{}' # capture payload.hash
culturabuilder gateway call config.patch --params '{
...
```json5
// ~/.culturabuilder/culturabuilder.json
{
  gateway: { port: 18789 },
...
{ "$include": "/etc/culturabuilder/base.json5" } // absolute
...
```json5
// ~/.culturabuilder/clients/mueller/agents.json5
[
...
```json5
// ~/.culturabuilder/clients/mueller/broadcast.json5
{
...
Culturabuilder reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Culturabuilder runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `CULTURABUILDER_LOAD_SHELL_ENV=1`
- `CULTURABUILDER_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${CULTURABUILDER_GATEWAY_TOKEN}"
    }
  }
}
```
...
Culturabuilder stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.culturabuilder/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.culturabuilder/credentials/oauth.json` (or `$CULTURABUILDER_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.culturabuilder/agent/*` (migrated by `culturabuilder doctor` into `~/.culturabuilder/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `CULTURABUILDER_OAUTH_DIR`
- Agent dir (default agent root override): `CULTURABUILDER_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Culturabuilder imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Culturabuilder derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/culturabuilder/culturabuilder-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/culturabuilder/culturabuilder.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/culturabuilder/culturabuilder.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.culturabuilder/credentials/whatsapp/biz
          // authDir: "~/.culturabuilder/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `culturabuilder doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "culturabuilder", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.culturabuilder/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `culturabuilder.json`).
...
Culturabuilder starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Culturabuilder starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `culturabuilder plugins install @culturabuilder/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Culturabuilder starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Culturabuilder spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Culturabuilder
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/culturabuilder" } }
}
```
...
When a file exceeds this limit, Culturabuilder logs a warning and injects a truncated
...
message envelopes). If unset, Culturabuilder uses the host timezone at runtime.
...
`[culturabuilder]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[culturabuilder]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[culturabuilder]"`, Culturabuilder will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Culturabuilder generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.culturabuilder/settings/tts.json",
      elevenlabs: {
...
Culturabuilder also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Culturabuilder currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `culturabuilder configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:culturabuilder`: all built-in Culturabuilder tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.culturabuilder/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.culturabuilder/sandboxes",
        docker: {
        ...
        browser: {
          enabled: false,
          image: "culturabuilder-sandbox-browser:bookworm-slim",
          containerPrefix: "culturabuilder-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Culturabuilder uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.culturabuilder/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Culturabuilder config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Culturabuilder writes/merges a `models.json` into
`~/.culturabuilder/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Culturabuilder uses
...
- Shortcut: `culturabuilder onboard --auth-choice opencode-zen`.
...
Shortcut: `culturabuilder onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `culturabuilder onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `culturabuilder onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `culturabuilder onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `culturabuilder onboard --auth-choice minimax-api`.
...
- Override the agent config root with `CULTURABUILDER_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.culturabuilder/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.culturabuilder/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.culturabuilder/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Culturabuilder stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.culturabuilder/skills` (workspace skills
...
from `~/.culturabuilder/extensions`, `<workspace>/.culturabuilder/extensions`, plus any
...
Culturabuilder can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Culturabuilder.app menubar, or `culturabuilder gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/culturabuilder" }
...
- Examples: `"/ui"`, `"/culturabuilder"`, `"/apps/culturabuilder"`.
...
- When a connection comes from one of these IPs, Culturabuilder uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `culturabuilder gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `CULTURABUILDER_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `CULTURABUILDER_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Culturabuilder
...
macOS app behavior:
- Culturabuilder.app watches `~/.culturabuilder/culturabuilder.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.culturabuilder/culturabuilder.json` (or `CULTURABUILDER_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.culturabuilder/culturabuilder.json` (or `CULTURABUILDER_CONFIG_PATH`)
...
- `CULTURABUILDER_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `culturabuilder --dev …` → uses `~/.culturabuilder-dev` + shifts ports from base `19001`
- `culturabuilder --profile <name> …` → uses `~/.culturabuilder-<name>` (port via config/env/flags)
...
Example:
```bash
CULTURABUILDER_CONFIG_PATH=~/.culturabuilder/a.json \
CULTURABUILDER_STATE_DIR=~/.culturabuilder-a \
culturabuilder gateway --port 19001
```
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.culturabuilder/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
- `x-culturabuilder-token: <token>` **or**
- `?token=<token>`
...
Gmail helper config (used by `culturabuilder webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "culturabuilder@gmail.com",
...
- Set `CULTURABUILDER_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Culturabuilder defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__culturabuilder/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__culturabuilder__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `CULTURABUILDER_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.culturabuilder/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.culturabuilder/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.culturabuilder/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_culturabuilder-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_culturabuilder-bridge._tcp` under `~/.culturabuilder/dns/` using the standard discovery domain `culturabuilder.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `culturabuilder.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `culturabuilder.internal` via that server

One-time setup helper (gateway host):

```bash
culturabuilder dns setup --apply
```
