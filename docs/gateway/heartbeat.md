...
- During heartbeat runs, Culturabuilder treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `culturabuilder sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Culturabuilder skips the heartbeat run entirely (no model call).
...
```bash
culturabuilder system event --text "Check for urgent follow-ups" --mode now
```
