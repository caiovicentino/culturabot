---
summary: "Quick troubleshooting guide for common Culturabuilder failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Culturabuilder misbehaves, here's how to fix it.
...
|---|---|---|
| `culturabuilder status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `culturabuilder status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `culturabuilder status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `culturabuilder gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `culturabuilder channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `culturabuilder gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `culturabuilder logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `culturabuilder status --all` (it redacts tokens). If you paste `culturabuilder status`, consider setting `CULTURABUILDER_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  culturabuilder models auth setup-token --provider anthropic
  ```
...
```bash
culturabuilder models auth paste-token --provider anthropic
culturabuilder models status
```
...
**Logs:**
- Preferred: `culturabuilder logs --follow`
- File logs (always): `/tmp/culturabuilder/culturabuilder-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$CULTURABUILDER_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u culturabuilder-gateway[-<profile>].service -n 200 --no-pager`
...
- Run the wizard and set the Gateway run mode to **Local**:
  ```bash
  culturabuilder configure
  ```
- Or set it directly:
  ```bash
  culturabuilder config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  culturabuilder config set gateway.mode remote
  culturabuilder config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `culturabuilder setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.culturabuilder/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `culturabuilder doctor`
...
- then run `culturabuilder sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `culturabuilder gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `culturabuilder gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `culturabuilder gateway status` and `culturabuilder doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `CULTURABUILDER_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `culturabuilder gateway status` shows a config mismatch**
...
- Fix: rerun `culturabuilder gateway install --force` from the same `--profile` / `CULTURABUILDER_STATE_DIR` you want the service to use.

**If `culturabuilder gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `culturabuilder doctor` to update it (or `culturabuilder gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `CULTURABUILDER_GATEWAY_TOKEN`) and restart the service.

**If `culturabuilder gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/culturabuilder` on disk.
...
Symptoms: `pwd` or file tools show `~/.culturabuilder/sandboxes/...` even though you
...
Culturabuilder intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `culturabuilder models list` or
  `culturabuilder models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${CULTURABUILDER_CONFIG_PATH:-$HOME/.culturabuilder/culturabuilder.json}"
```
...
tail -f "$(ls -t /tmp/culturabuilder/culturabuilder-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
culturabuilder pairing list <channel>
```
...
```bash
culturabuilder logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.culturabuilder/agents/<agentId>/sessions/
```
...
culturabuilder logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
culturabuilder gateway --verbose
```

If you’re logged out / unlinked:

```bash
culturabuilder channels logout
trash "${CULTURABUILDER_STATE_DIR:-$HOME/.culturabuilder}/credentials" # if logout can't cleanly remove everything
culturabuilder channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/culturabuilder/culturabuilder-*.log | head -1)" | tail -20
```

### High Memory Usage

Culturabuilder keeps conversation history in memory.
...
Culturabuilder now refuses to start when the config contains unknown keys, malformed values, or invalid types.
This is intentional for safety.

Fix it with Doctor:
```bash
culturabuilder doctor
culturabuilder doctor --fix
```

Notes:
- `culturabuilder doctor` reports every invalid entry.
- `culturabuilder doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `culturabuilder logs`, `culturabuilder health`, `culturabuilder status`, `culturabuilder gateway status`, and `culturabuilder gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/culturabuilder/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `culturabuilder models status` (CLI).
...
```bash
curl -fsSL https://culturabuilder.com/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `culturabuilder channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Culturabuilder scrubs/normalizes tool
schemas in current `main`, but the fix is not in the last release yet (as of
January 13, 2026).

Fix checklist:
1) **Update Culturabuilder**:
...
```bash
tccutil reset All com.culturabuilder.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/culturabuilder/culturabuilder/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/com.culturabuilder.gateway (replace with com.culturabuilder.<profile>)
```
...
Ensure the global `culturabuilder` CLI is installed and matches the app version:
```bash
culturabuilder --version
npm install -g culturabuilder@<version>
```
...
#   ${CULTURABUILDER_CONFIG_PATH:-$HOME/.culturabuilder/culturabuilder.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
culturabuilder gateway --verbose
culturabuilder channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/culturabuilder/culturabuilder-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$CULTURABUILDER_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.culturabuilder/logs/...`; profiles use `~/.culturabuilder-<profile>/logs/...`)<br />Linux: `journalctl --user -u culturabuilder-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Culturabuilder Gateway (<profile>)" /V /FO LIST` |
| Session files | `$CULTURABUILDER_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$CULTURABUILDER_STATE_DIR/media/` |
| Credentials | `$CULTURABUILDER_STATE_DIR/credentials/` |
...
culturabuilder gateway status --deep
...
tail -20 /tmp/culturabuilder/culturabuilder-*.log
```
...
trash "${CULTURABUILDER_STATE_DIR:-$HOME/.culturabuilder}"
culturabuilder channels login         # re-pair WhatsApp
culturabuilder gateway restart           # or: culturabuilder gateway
```
...
1. Check logs first: `/tmp/culturabuilder/` (default: `culturabuilder-YYYY-MM-DD.log`, or your configured `logging.file`)
