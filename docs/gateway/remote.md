...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Culturabuilder runs”).
...
With the tunnel up:
- `culturabuilder health` and `culturabuilder status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `culturabuilder gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`CULTURABUILDER_GATEWAY_PORT`).
