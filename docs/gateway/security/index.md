...
## Quick check: `culturabuilder security audit` (formerly `culturabuilder security audit`)
...
# (On older installs, the command is `culturabuilder ...`.)
```
...
- Tighten local perms (`~/.culturabuilder` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... *spicy*. Here’s how to not get pwned.

Culturabuilder is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Culturabuilder also attempts a best-effort live Gateway probe.
...
`culturabuilder security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${CULTURABUILDER_GATEWAY_PASSWORD}
```
...
Culturabuilder stores session transcripts on disk under `~/.culturabuilder/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.culturabuilder` (see the audit section below). If you need
...
Culturabuilder can refresh the skills list mid-session:
...
- If you install plugins from npm (`culturabuilder plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.culturabuilder/extensions/<pluginId>/` (or `$CULTURABUILDER_STATE_DIR/extensions/<pluginId>/`).
  - Culturabuilder uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
culturabuilder pairing approve <channel> <code>
```
...
By default, Culturabuilder routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Culturabuilder has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.culturabuilder/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.culturabuilder or your logs.”
...
4. **Re-run audit**
   - `culturabuilder security audit --deep` and confirm the report is clean.
...
- `~/.culturabuilder/culturabuilder.json`: `600` (user read/write only)
- `~/.culturabuilder`: `700` (user only)

`culturabuilder doctor` can warn and offer to tighten these permissions.

### 0.4) Network exposure (bind + port + firewall)

The Gateway multiplexes **WebSocket + HTTP** on a single port:
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `CULTURABUILDER_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_culturabuilder-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `CULTURABUILDER_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `culturabuilder doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `CULTURABUILDER_GATEWAY_PASSWORD`).

Rotation checklist (token/password):
1. Generate/set a new secret (`gateway.auth.token` or `CULTURABUILDER_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Culturabuilder
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Culturabuilder verifies the identity by resolving the
...
- Culturabuilder will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.culturabuilder/` (or `$CULTURABUILDER_STATE_DIR/`) may contain secrets or private data:

- `culturabuilder.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `culturabuilder status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.culturabuilder/sandboxes`
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `culturabuilder gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `CULTURABUILDER_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/culturabuilder/culturabuilder-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.culturabuilder/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Culturabuilder version
...
Found a vulnerability in Culturabuilder? Please report responsibly:
