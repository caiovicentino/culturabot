...
- Plugin manifests (`culturabuilder.plugin.json`) are required for all plugins.

## Plugin schema enforcement
- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1) Resolve plugin manifest + schema (`culturabuilder.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `culturabuilder doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `culturabuilder doctor --fix`.”
