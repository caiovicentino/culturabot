...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.culturabuilder/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.culturabuilder/node.json` (node id + pairing token).
...
Path: `~/.culturabuilder/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.culturabuilder/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.culturabuilder/exec-approvals.sock` (0600).
