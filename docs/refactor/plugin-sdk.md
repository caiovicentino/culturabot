...
Delivery:
- Publish as `@culturabuilder/plugin-sdk` (or export from core under `culturabuilder/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `CulturabuilderPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: CulturabuilderConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: CulturabuilderConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: CulturabuilderConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: CulturabuilderConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: CulturabuilderConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: CulturabuilderConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@culturabuilder/plugin-sdk`.
- Add `api.runtime` to `CulturabuilderPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `culturabuilderRuntime: ">=2026.2.0"`).
