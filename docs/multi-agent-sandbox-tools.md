...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `culturabuilder sandbox explain`.
...
Legacy `agent.*` configs are migrated by `culturabuilder doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=culturabuilder.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${CULTURABUILDER_STATE_DIR:-$HOME/.culturabuilder}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
