...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Culturabuilder
---

# Node + tsx "__name is not a function" crash

## Summary
Running Culturabuilder via Node with `tsx` fails at startup with:

```
[culturabuilder] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `culturabuilder status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch culturabuilder.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node culturabuilder.mjs status` works on Node 25.
