---
summary: "Chrome extension: let Culturabuilder drive your existing Chrome tab"
read_when:
...
The Culturabuilder Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
Culturabuilder then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Culturabuilder release (npm package) as static files. There is no separate “build” step.

After upgrading Culturabuilder:
- Re-run `culturabuilder browser extension install` to refresh the installed files under your Culturabuilder state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Culturabuilder ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Culturabuilder to control.
...
- `ON`: attached; Culturabuilder can drive that tab.
...
Debugging: `culturabuilder sandbox explain`
...
`culturabuilder browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `culturabuilder browser extension install` first to copy the extension to a stable location under your Culturabuilder state directory.
