...
- **Session store** (chat history + routing state) under `~/.culturabuilder/agents/<agentId>/sessions`.
...
available from `~/.culturabuilder/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.culturabuilder` (or `CULTURABUILDER_STATE_DIR`)
- Workspace: `~/clawd` (or `~/clawd-<agentId>`)
- Agent dir: `~/.culturabuilder/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.culturabuilder/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Culturabuilder runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/clawd` (or `~/clawd-<profile>` when `CULTURABUILDER_PROFILE` is set).
- State defaults to `~/.culturabuilder/agents/main/agent`.
...
```bash
culturabuilder agents list --bindings
```
...
`~/.culturabuilder/culturabuilder.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        agentDir: "~/.culturabuilder/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/clawd-work",
        agentDir: "~/.culturabuilder/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.culturabuilder/credentials/whatsapp/personal
          // authDir: "~/.culturabuilder/credentials/whatsapp/personal",
        },
        biz: {
