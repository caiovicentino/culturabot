...
- `/usage cost` in chats: local cost summary aggregated from Culturabuilder session logs.
- CLI: `culturabuilder status --usage` prints a full per-provider breakdown.
- CLI: `culturabuilder channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
