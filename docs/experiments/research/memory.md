...
read_when:
  ...
  - Deciding: standalone CLI vs deep Culturabuilder integration
...
Recommendation: **deep integration in Culturabuilder**, but keep a separable core library.

### Why integrate into Culturabuilder?
- Culturabuilder already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `culturabuilder memory recall "…" --k 25 --since 30d`
  - `culturabuilder memory reflect --since 7d`
