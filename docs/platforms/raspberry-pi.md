---
summary: "Culturabuilder on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Culturabuilder on a Raspberry Pi
  - Running Culturabuilder on ARM devices
  - Building a cheap always-on personal AI
---

# Culturabuilder on Raspberry Pi

## Goal

Run a persistent, always-on Culturabuilder Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Culturabuilder features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled culturabuilder

# Enable if not
sudo systemctl enable culturabuilder

# Start on boot
sudo systemctl start culturabuilder
```
...
```bash
# Check logs
journalctl -u culturabuilder --no-pager -n 100

# Common fix: rebuild
cd ~/culturabuilder  # if using hackable install
npm run build
sudo systemctl restart culturabuilder
```
