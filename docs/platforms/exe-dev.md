---
summary: "Run Culturabuilder Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: Culturabuilder Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install Culturabuilder  
2) Run `culturabuilder onboard --install-daemon`  
...
```bash
ssh exe.dev new --name=culturabuilder
```

Then connect:

```bash
ssh culturabuilder.exe.xyz
```

Tip: keep this VM **stateful**. Culturabuilder stores state under `~/.culturabuilder/` and `~/clawd/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 culturabuilder.exe.xyz
```
...
```bash
export CULTURABUILDER_GATEWAY_TOKEN="$(openssl rand -hex 32)"
culturabuilder gateway --bind lan --port 8080 --token "$CULTURABUILDER_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.culturabuilder/culturabuilder.json`:
...
Notes:
- Non-loopback binds require `gateway.auth.token` (or `CULTURABUILDER_GATEWAY_TOKEN`).
- `gateway.remote.token` is only for remote CLI calls; it does not enable local auth.

Then point exe.dev’s proxy at `8080` (or whatever port you chose) and open your VM’s HTTPS URL:

```bash
ssh exe.dev share port culturabuilder 8080
```

Open:
- `https://culturabuilder.exe.xyz/`
...
On Linux, Culturabuilder uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status culturabuilder-gateway[-<profile>].service
```
