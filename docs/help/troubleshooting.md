...
### `culturabuilder: command not found`
...
You can also set `CULTURABUILDER_VERBOSE=1` instead of the flag.
...
### `docs.culturabuilder.com` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.culturabuilder.com` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.culturabuilder.com` to the allowlist, then retry.
...
- Check the allowlist: `culturabuilder config get agents.defaults.models`
...
If you can, include the relevant log tail from `culturabuilder logs --follow`.
