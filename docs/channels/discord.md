...
3) Set the token for Culturabuilder:
...
## Goals
- Talk to Culturabuilder via Discord DMs or guild channels.
...
3. Configure Culturabuilder with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `culturabuilder pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running Culturabuilder in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Culturabuilder needs
...
Discord uses numeric ids everywhere; Culturabuilder config prefers ids.
...
### Troubleshooting
- First: run `culturabuilder doctor` and `culturabuilder channels status --probe` (actionable warnings + quick audits).
...
- On startup, Culturabuilder resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror Culturabuilder’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Culturabuilder enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`culturabuilder gateway --force`) after confirming no other processes own the Discord session.
