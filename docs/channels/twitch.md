...
read_when:
  - Setting up Twitch chat integration for Culturabuilder
---
# Twitch (plugin)

Twitch chat support via IRC connection. Culturabuilder connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
culturabuilder plugins install @culturabuilder/twitch
```

Local checkout (when running from a git repo):

```bash
culturabuilder plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `CULTURABUILDER_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "culturabuilder",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use CULTURABUILDER_TWITCH_ACCESS_TOKEN env var)
...
```bash
CULTURABUILDER_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
