...
3) Configure Culturabuilder with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for Culturabuilder + `imsg` (Messages DB access).
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. Culturabuilder only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. Culturabuilder can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, Culturabuilder attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - culturabuilder gateway           │          SCP (attachments)        │ - Messages signed in     │
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.culturabuilder/scripts/imsg-ssh",
...
Example wrapper (`~/.culturabuilder/scripts/imsg-ssh`):
```bash
#!/usr/bin/env bash
...
Multi-account support: use `channels.imessage.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) for the shared pattern. Don't commit `~/.culturabuilder/culturabuilder.json` (it often contains tokens).
...
- Approve via:
  - `culturabuilder pairing list imessage`
  - `culturabuilder pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, Culturabuilder treats that thread as a “group” for:
