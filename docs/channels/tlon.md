...
Tlon is a decentralized messenger built on Urbit. Culturabuilder connects to your Urbit ship and can
...
```bash
culturabuilder plugins install @culturabuilder/tlon
```

Local checkout (when running from a git repo):

```bash
culturabuilder plugins install ./extensions/tlon
```
...
Use these with `culturabuilder message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Culturabuilder replies in-thread.
