...
```bash
culturabuilder plugins install @culturabuilder/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
culturabuilder plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Culturabuilder" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `culturabuilder pairing list nextcloud-talk`
  - `culturabuilder pairing approve nextcloud-talk <CODE>`
