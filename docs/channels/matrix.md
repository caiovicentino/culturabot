...
Matrix is an open, decentralized messaging protocol. Culturabuilder connects as a Matrix **user**
...
```bash
culturabuilder plugins install @culturabuilder/matrix
```

Local checkout (when running from a git repo):

```bash
culturabuilder plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `culturabuilder plugins install @culturabuilder/matrix`
   - From a local checkout: `culturabuilder plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Culturabuilder calls the same
     login endpoint, stores the access token in `~/.culturabuilder/credentials/matrix/credentials.json`,
...
- On first connection, Culturabuilder requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Culturabuilder logs a warning.
...
`~/.culturabuilder/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `culturabuilder pairing list matrix`
  - `culturabuilder pairing approve matrix <CODE>`
...
- On startup, Culturabuilder resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
