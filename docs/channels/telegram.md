...
- If Telegram rejects the HTML payload, Culturabuilder retries the same message as plain text.

## Commands (native + custom)
Culturabuilder registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; Culturabuilder does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `culturabuilder logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.

## Config writes
By default, Telegram is allowed to write config updates triggered by channel events or `/config set|unset`.

This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). Culturabuilder can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. Culturabuilder:
...
Private chats can include `message_thread_id` in some edge cases. Culturabuilder keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2) Run `culturabuilder logs --follow` and look for `from.id`.
...
Culturabuilder defaults to audio files for backward compatibility.
...
Culturabuilder supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, Culturabuilder handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, Culturabuilder caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.culturabuilder/telegram/sticker-cache.json`
...
Culturabuilder uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, Culturabuilder:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by Culturabuilder)
...
- Example: `culturabuilder message send --channel telegram --target 123456789 --message "hi"`.
...
- `culturabuilder channels status` shows a warning when config expects unmentioned group messages.
- `culturabuilder channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `culturabuilder logs --follow` (look for "skipping group message")
...
- Upgrade to a Culturabuilder build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
