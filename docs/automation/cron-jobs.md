...
- Jobs persist under `~/.culturabuilder/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.culturabuilder/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.culturabuilder/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.culturabuilder/cron/jobs.json",
...
- `CULTURABUILDER_SKIP_CRON=1` (env)

## CLI quickstart

One-shot reminder (UTC ISO, auto-delete after success):
```bash
culturabuilder cron add \
  --name "Send reminder" \
...
culturabuilder cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
culturabuilder cron edit <jobId> --agent ops
culturabuilder cron edit <jobId> --clear-agent
```
```
...
```bash
culturabuilder cron edit <jobId> \
...
```bash
culturabuilder system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`culturabuilder system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `CULTURABUILDER_SKIP_CRON`.
