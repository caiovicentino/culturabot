...
Deploy Culturabuilder on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/culturabuilder/culturabuilder" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run Culturabuilder:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: CULTURABUILDER_STATE_DIR
        value: /data/.culturabuilder
      - key: CULTURABUILDER_WORKSPACE_DIR
        value: /data/workspace
      - key: CULTURABUILDER_GATEWAY_TOKEN
        generateValue: true  # auto-generates a secure token
    disk:
      name: culturabuilder-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/culturabuilder`.
...
If you use the original Culturabuilder repository, Render will not auto-deploy your Culturabuilder. To update it, run a manual Blueprint sync from the dashboard.
...
For Culturabuilder, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Culturabuilder host.
