...
Culturabuilder features that can generate provider usage or paid API calls.
...
- `culturabuilder status --usage` and `culturabuilder channels list` show provider **usage windows**
...
Culturabuilder can pick up credentials from:
...
- `culturabuilder status --usage`
- `culturabuilder models status --json`
...
### 8) Model scan / probe
`culturabuilder models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
