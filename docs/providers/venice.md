---
summary: "Use Venice AI privacy-focused models in Culturabuilder"
read_when:
  - You want privacy-focused inference in Culturabuilder
...
```bash
culturabuilder onboard --auth-choice venice-api-key
```
...
```bash
culturabuilder chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Culturabuilder shows all available Venice models. Pick based on your needs:
...
```bash
culturabuilder models set venice/claude-opus-45
culturabuilder models set venice/llama-3.3-70b
```

List all available models:

```bash
culturabuilder models list | grep venice
```

## Configure via `culturabuilder configure`

1. Run `culturabuilder configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Culturabuilder automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
culturabuilder chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
culturabuilder chat --model venice/claude-opus-45

# Use uncensored model
culturabuilder chat --model venice/venice-uncensored

# Use vision model with image
culturabuilder chat --model venice/qwen3-vl-235b-a22b

# Use coding model
culturabuilder chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `culturabuilder models list` to see currently available models. Some models may be temporarily offline.
