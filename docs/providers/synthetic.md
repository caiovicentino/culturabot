---
summary: "Use Synthetic's Anthropic-compatible API in Culturabuilder"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Culturabuilder registers it as the
...
```bash
culturabuilder onboard --auth-choice synthetic-api-key
```
...
Note: Culturabuilder's Anthropic client appends `/v1` to the base URL, so use
