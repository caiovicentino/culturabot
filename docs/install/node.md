...
read_when:
  - "You installed Culturabuilder but `culturabuilder` is “command not found”"
...
Culturabuilder’s runtime baseline is **Node 22+**.

If you can run `npm install -g culturabuilder@latest` but later see `culturabuilder: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
