---
summary: "First-run onboarding flow for Culturabuilder (macOS app)"
read_when:
...
- Writes credentials to `~/.culturabuilder/credentials/oauth.json`
...
The app can install the global `culturabuilder` CLI via npm/pnpm so terminal
...
On the first agent run, Culturabuilder bootstraps a workspace (default `~/clawd`):
...
```bash
culturabuilder webhooks gmail setup --account you@gmail.com
```
...
- `~/.culturabuilder/credentials/oauth.json`
