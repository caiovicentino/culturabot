...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Culturabuilder would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Culturabuilder would reply in this chat, it will run all three agents.
...
**Debug:**
```bash
tail -f ~/.culturabuilder/logs/gateway.log | grep broadcast
```
...
```typescript
interface CulturabuilderConfig {
  broadcast?: {
