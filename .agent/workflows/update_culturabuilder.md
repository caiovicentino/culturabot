---
description: Update Culturabuilder from upstream when branch has diverged (ahead/behind)
---

# Culturabuilder Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Culturabuilder" || true

# Move old version
mv /Applications/Culturabuilder.app /tmp/Culturabuilder-backup.app

# Install new build
cp -R dist/Culturabuilder.app /Applications/

# Launch
open /Applications/Culturabuilder.app
```
...
pnpm culturabuilder health

# Verify no zombie processes
ps aux | grep -E "(culturabuilder|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm culturabuilder agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update culturabuilder.json with fallback chains
...
pnpm culturabuilder agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
