...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("CULTURABUILDER_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.CULTURABUILDER_STATE_DIR = path.join(base, ".culturabuilder");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "culturabuilder-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".culturabuilder", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
