...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `culturabuilder-e2e-${name}-`));
  const configDir = path.join(homeDir, ".culturabuilder");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          CULTURABUILDER_CONFIG_PATH: configPath,
          CULTURABUILDER_STATE_DIR: stateDir,
          CULTURABUILDER_GATEWAY_TOKEN: "",
          CULTURABUILDER_GATEWAY_PASSWORD: "",
          CULTURABUILDER_SKIP_CHANNELS: "1",
          CULTURABUILDER_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        CULTURABUILDER_GATEWAY_TOKEN: inst.gatewayToken,
        CULTURABUILDER_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          CULTURABUILDER_GATEWAY_PORT: String(gwA.port),
          CULTURABUILDER_GATEWAY_TOKEN: gwA.gatewayToken,
          CULTURABUILDER_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          CULTURABUILDER_GATEWAY_PORT: String(gwB.port),
          CULTURABUILDER_GATEWAY_TOKEN: gwB.gatewayToken,
          CULTURABUILDER_GATEWAY_PASSWORD: "",
        }),
