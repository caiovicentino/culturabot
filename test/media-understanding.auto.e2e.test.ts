...
import type { CulturabuilderConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("culturabuilder-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("culturabuilder-bin-sherpa-");
      const modelDir = await makeTempDir("culturabuilder-sherpa-model-");
      ...
      const cfg: CulturabuilderConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("culturabuilder-bin-whispercpp-");
      const modelDir = await makeTempDir("culturabuilder-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("culturabuilder-bin-gemini-");
      ...
      const cfg: CulturabuilderConfig = { tools: { media: { image: {} } } };
