# Culturabuilder ACP Bridge

This document describes how the Culturabuilder ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`culturabuilder acp` exposes an ACP agent over stdio and forwards prompts to a running
Culturabuilder Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Culturabuilder Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `culturabuilder acp` over stdio.

Example config:

```bash
culturabuilder config set gateway.remote.url wss://gateway-host:18789
culturabuilder config set gateway.remote.token <token>
```

Example run:

```bash
culturabuilder acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
culturabuilder acp --session agent:main:main
culturabuilder acp --session agent:design:main
culturabuilder acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Culturabuilder ACP” to start a thread.

## Execution Model

- ACP client spawns `culturabuilder acp` and speaks ACP messages over stdio.
...
```bash
culturabuilder acp --session agent:main:main
culturabuilder acp --session-label "support inbox"
culturabuilder acp --reset-session
```
...
`culturabuilder acp` resolves the Gateway URL and auth from CLI flags or config:
