...
import "@a2ui/lit/ui";
import { themeContext } from "@culturabuilder/a2ui-theme-context";
...
class CulturabuilderA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  #themeProvider = new ContextProvider(this, {
    ...
    initialValue: culturabuilderTheme,
  });
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--culturabuilder-a2ui-inset-top, 0px)
        var(--culturabuilder-a2ui-inset-right, 0px)
        var(--culturabuilder-a2ui-inset-bottom, 0px)
        var(--culturabuilder-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--culturabuilder-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--culturabuilder-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--culturabuilder-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--culturabuilder-a2ui-empty-top, var(--culturabuilder-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.culturabuilderA2UI = api;
    globalThis.culturabuilderA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    globalThis.addEventListener("culturabuilder:a2ui-action-status", this.#statusListener);
    this.#syncSurfaces();
  }
  ...
  disconnectedCallback() {
    ...
    if (this.#statusListener) {
      globalThis.removeEventListener("culturabuilder:a2ui-action-status", this.#statusListener);
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__culturabuilderLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.culturabuilderCanvasA2UIAction ??
      globalThis.webkit?.messageHandlers?.culturabuilderCanvasA2UIAction ??
      globalThis.culturabuilderCanvasA2UIAction ??
      globalThis.culturabuilderCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (
          handler === globalThis.culturabuilderCanvasA2UIAction ||
          handler === globalThis.culturabuilderCanvasA2UIAction
        ) {
...
customElements.define("culturabuilder-a2ui-host", CulturabuilderA2UIHost);
