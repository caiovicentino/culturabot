...
private let chatUILogger = Logger(subsystem: "com.culturabuilder", category: "CulturabuilderChatUI")

@MainActor
@Observable
public final class CulturabuilderChatViewModel {
    public private(set) var messages: [CulturabuilderChatMessage] = []
    ...
    public var attachments: [CulturabuilderPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [CulturabuilderChatPendingToolCall] = []
    public private(set) var sessions: [CulturabuilderChatSessionEntry] = []
    private let transport: any CulturabuilderChatTransport
    ...
    private var pendingToolCallsById: [String: CulturabuilderChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any CulturabuilderChatTransport) {
    ...
    public var sessionChoices: [CulturabuilderChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [CulturabuilderChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [CulturabuilderChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: CulturabuilderPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [CulturabuilderChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CulturabuilderChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [CulturabuilderChatMessage]) -> [CulturabuilderChatMessage] {
        var result: [CulturabuilderChatMessage] = []
    ...
    private static func dedupeKey(for message: CulturabuilderChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [CulturabuilderChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> CulturabuilderChatAttachmentPayload in
            CulturabuilderChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> CulturabuilderChatSessionEntry {
        CulturabuilderChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: CulturabuilderChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: CulturabuilderChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: CulturabuilderAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CulturabuilderChatPendingToolCall(
    ...
    private func addImageAttachment(url: URL?, data: Data, fileName: String, mimeType: String) async {
        ...
        self.attachments.append(
            CulturabuilderPendingAttachment(
                url: url,
                data: data,
                fileName: fileName,
                mimeType: mimeType,
    ...
    private static func previewImage(data: Data) -> CulturabuilderPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
