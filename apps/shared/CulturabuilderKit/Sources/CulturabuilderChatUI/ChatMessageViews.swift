...
struct ChatMessageBubble: View {
    let message: CulturabuilderChatMessage
    let style: CulturabuilderChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? CulturabuilderChatTheme.userText : CulturabuilderChatTheme.assistantText
    ...
    private var inlineAttachments: [CulturabuilderChatMessageContent] {
    ...
    private var toolCalls: [CulturabuilderChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [CulturabuilderChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? CulturabuilderChatTheme.userBubble
        }
        if self.style == .onboarding {
            return CulturabuilderChatTheme.onboardingAssistantBubble
        }
        return CulturabuilderChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return CulturabuilderChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: CulturabuilderChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? CulturabuilderChatTheme.userText : CulturabuilderChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: CulturabuilderChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(CulturabuilderChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            if self.style == .standard {
                Text("Culturabuilder is thinking…")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                Spacer()
            }
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(CulturabuilderChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [CulturabuilderChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: CulturabuilderChatTheme.assistantText)
            }
        }
    }
}
