...
@MainActor
struct CulturabuilderChatComposer: View {
    @Bindable var viewModel: CulturabuilderChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(CulturabuilderChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(CulturabuilderChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \CulturabuilderPendingAttachment.id)
                { (att: CulturabuilderPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            CulturabuilderPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(CulturabuilderChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(CulturabuilderChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(CulturabuilderChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(CulturabuilderChatTheme.subtleCard)
        .clipShape(Capsule())
    }
