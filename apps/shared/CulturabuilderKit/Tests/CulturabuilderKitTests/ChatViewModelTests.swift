...
import Foundation
import Testing
@testable import CulturabuilderChatUI
...
private final class TestChatTransport: @unchecked Sendable, CulturabuilderChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CulturabuilderChatHistoryPayload]
    private let sessionsResponses: [CulturabuilderChatSessionsListResponse]

    private let stream: AsyncStream<CulturabuilderChatTransportEvent>
    private let continuation: AsyncStream<CulturabuilderChatTransportEvent>.Continuation

    init(
        historyResponses: [CulturabuilderChatHistoryPayload],
        sessionsResponses: [CulturabuilderChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<CulturabuilderChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> CulturabuilderChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CulturabuilderChatHistoryPayload(
        ...
        attachments _: [CulturabuilderChatAttachmentPayload]) async throws -> CulturabuilderChatSendResponse
    {
        ...
        return CulturabuilderChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> CulturabuilderChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? CulturabuilderChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: CulturabuilderChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = CulturabuilderChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = CulturabuilderChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { CulturabuilderChatViewModel(sessionKey: "main", transport: transport) }
        ...
        transport.emit(
            .agent(
                CulturabuilderAgentEventPayload(
                    runId: sessionId,
                    seq: 1,
                    stream: "assistant",
        ...
        transport.emit(
            .chat(
                CulturabuilderChatEventPayload(
                    runId: runId,
                    sessionKey: "main",
                    state: "final",
                    message: nil,
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = CulturabuilderChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = CulturabuilderChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = CulturabuilderChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { CulturabuilderChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
