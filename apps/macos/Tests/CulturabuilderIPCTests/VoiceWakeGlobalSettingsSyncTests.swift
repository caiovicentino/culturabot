...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = CulturabuilderProtocol.AnyCodable(["triggers": ["culturabuilder", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["culturabuilder", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = CulturabuilderProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
