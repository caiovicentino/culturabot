...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: CulturabuilderProtocol.AnyCodable(["ok": CulturabuilderProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: CulturabuilderProtocol.AnyCodable(["ok": CulturabuilderProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = CulturabuilderProtocol.AnyCodable([
            "runId": CulturabuilderProtocol.AnyCodable("run-1"),
            "sessionKey": CulturabuilderProtocol.AnyCodable("main"),
            "state": CulturabuilderProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: CulturabuilderProtocol.AnyCodable(["a": CulturabuilderProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
