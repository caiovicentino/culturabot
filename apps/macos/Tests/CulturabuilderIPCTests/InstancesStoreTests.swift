...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: CulturabuilderProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CulturabuilderProtocol.AnyCodable] = [
            "presence": .init([CulturabuilderProtocol.AnyCodable(entry)]),
        ]
        let payload = CulturabuilderProtocol.AnyCodable(payloadMap)
