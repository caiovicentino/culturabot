...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "com.culturabuilder", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in CulturabuilderBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: CulturabuilderBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "com.culturabuilder.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        let domain = CulturabuilderBonjour.wideAreaGatewayServiceDomain
    ...
    private func updateGateways(for domain: String) {
        ...
        if domain == CulturabuilderBonjour.wideAreaGatewayServiceDomain,
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Culturabuilder)", with: "")
