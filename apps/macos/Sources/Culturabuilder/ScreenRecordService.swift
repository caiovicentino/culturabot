...
final class ScreenRecordService {
    ...
    private let logger = Logger(subsystem: "com.culturabuilder", category: "screenRecord")
        ...
        outPath: String?) async throws -> (path: String, hasAudio: Bool)
    {
        ...
        let outURL: URL = {
            ...
            return FileManager().temporaryDirectory
                .appendingPathComponent("culturabuilder-screen-record-\(UUID().uuidString).mp4")
        }()
...
private final class StreamRecorder: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    let queue = DispatchQueue(label: "com.culturabuilder.screenRecord.writer")
