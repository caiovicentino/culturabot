...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "com.culturabuilder", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> CulturabuilderSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: CulturabuilderSessionPreviewEntry,
    ...
    private static func cache(payload: CulturabuilderSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: CulturabuilderChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [CulturabuilderKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [CulturabuilderKit.AnyCodable]) -> [CulturabuilderChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(CulturabuilderChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: CulturabuilderChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: CulturabuilderChatMessage) -> Bool {
    ...
    private static func toolNames(for message: CulturabuilderChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: CulturabuilderChatMessage) -> String? {
