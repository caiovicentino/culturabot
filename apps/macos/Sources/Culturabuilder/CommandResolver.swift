...
enum CommandResolver {
    private static let projectRootDefaultsKey = "culturabuilder.gatewayProjectRootPath"
    private static let helperName = "culturabuilder"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/culturabuilder.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__CULTURABUILDER_ERR__' >&2
        \(message)
        __CULTURABUILDER_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/culturabuilder")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let culturabuilderPaths = self.culturabuilderManagedPaths(home: home)
        if !culturabuilderPaths.isEmpty {
            extras.insert(contentsOf: culturabuilderPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + culturabuilderPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func culturabuilderManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".culturabuilder")
    ...
    static func culturabuilderExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectCulturabuilderExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/culturabuilder.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyCulturabuilderInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.culturabuilderExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func culturabuilderNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let culturabuilderPath = self.projectCulturabuilderExecutable(projectRoot: root) {
                return [culturabuilderPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "culturabuilder", subcommand] + extraArgs
            }
            if let culturabuilderPath = self.culturabuilderExecutable(searchPaths: searchPaths) {
                return [culturabuilderPath, subcommand] + extraArgs
            }

            let missingEntry = """
            culturabuilder entrypoint missing (looked for dist/index.js or bin/culturabuilder.js); run pnpm build.
            """
    ...
    // Existing callers still refer to culturabuilderCommand; keep it as node alias.
    static func culturabuilderCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
        searchPaths: [String]? = nil) -> [String]
    {
        self.culturabuilderNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real culturabuilder CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/culturabuilder"
        ...
        \(cliSection)
        \(projectSection)
        if command -v culturabuilder >/dev/null 2>&1; then
          CLI="$(command -v culturabuilder)"
          culturabuilder \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/culturabuilder.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/culturabuilder.js"
            node "$PRJ/bin/culturabuilder.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent culturabuilder"
          pnpm --silent culturabuilder \(quotedArgs);
        else
          echo "culturabuilder CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? CulturabuilderConfigFile.loadDict()
