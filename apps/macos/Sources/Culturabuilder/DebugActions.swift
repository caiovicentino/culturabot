...
enum DebugActions {
    private static let verboseDefaultsKey = "culturabuilder.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "culturabuilder.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Culturabuilder", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".culturabuilder/culturabuilder.json")
        guard
