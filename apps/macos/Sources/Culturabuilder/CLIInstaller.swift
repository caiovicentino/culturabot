...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("culturabuilder").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing culturabuilder CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed culturabuilder \($0)." } ?? "Installed culturabuilder."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://culturabuilder.com/install-cli.sh | \
