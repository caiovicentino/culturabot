...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CULTURABUILDER_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/culturabuilder")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("culturabuilder-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("culturabuilder-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/culturabuilder/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("culturabuilder") && $0.pathExtension == "log" }
            .max { lhs, rhs in
