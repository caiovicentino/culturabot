...
final class GatewayProcessManager {
    ...
    #endif
    private let logger = Logger(subsystem: "com.culturabuilder", category: "gateway.process")
    ...
    private func describeAttachFailure(_ error: Error, port: Int, instance: PortGuardian.Descriptor?) -> String {
        ...
        if self.isGatewayAuthFailure(error) {
            return """
            Gateway on port \(port) rejected auth. Set gateway.auth.token (or CULTURABUILDER_GATEWAY_TOKEN) \
