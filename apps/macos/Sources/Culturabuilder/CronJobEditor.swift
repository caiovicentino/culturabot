...
struct CronJobEditor: View {
    ...
    static let introText =
        "Create a schedule that wakes culturabuilder via the Gateway. "
    ...
    static let sessionTargetNote =
        "Main jobs post a system event into the current main session. "
            + "Isolated jobs run culturabuilder in a dedicated session and can deliver results (WhatsApp/Telegram/Discord/etc)."
        ...
        VStack(alignment: .leading, spacing: 10) {
            Grid(alignment: .leadingFirstTextBaseline, horizontalSpacing: 14, verticalSpacing: 10) {
                GridRow {
                    ...
                    TextField("What should culturabuilder do?", text: self.$agentMessage, axis: .vertical)
                        .textFieldStyle(.roundedBorder)
                        .lineLimit(3...7)
                        .frame(maxWidth: .infinity)
                }
