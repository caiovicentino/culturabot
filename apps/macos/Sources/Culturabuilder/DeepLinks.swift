...
private let deepLinkLogger = Logger(subsystem: "com.culturabuilder", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Culturabuilder is paused", message: "Unpause Culturabuilder to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Culturabuilder agent?", message: body) else { return }
        }
