...
enum CulturabuilderLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = CulturabuilderOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CulturabuilderFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: labelSeparator) else {
            return ("com.culturabuilder", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        CulturabuilderLogging.bootstrapIfNeeded()
        let label = CulturabuilderLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CulturabuilderOSLogHandler: LogHandler {
...
struct CulturabuilderFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = CulturabuilderLogging.parseLabel(self.label)
