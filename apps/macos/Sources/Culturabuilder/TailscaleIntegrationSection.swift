...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.culturabuilder/culturabuilder.json. Prefer CULTURABUILDER_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.culturabuilder/culturabuilder.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.culturabuilder/culturabuilder.json. Restart the gateway to apply."
        }
