...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "com.culturabuilder", category: "portguard")
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("Culturabuilder", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "culturabuilder", "tsx", "pnpm", "bun"]
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            // The gateway daemon may listen as `culturabuilder` or as its runtime (`node`, `bun`, etc).
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a culturabuilder listener as expected.
            if cmd.contains("culturabuilder"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
