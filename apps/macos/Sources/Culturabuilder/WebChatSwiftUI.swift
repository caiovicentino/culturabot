...
private let webChatSwiftLogger = Logger(subsystem: "com.culturabuilder", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: CulturabuilderChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> CulturabuilderChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(CulturabuilderChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> CulturabuilderChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                CulturabuilderGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    CulturabuilderChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    CulturabuilderAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<CulturabuilderChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any CulturabuilderChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = CulturabuilderChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: CulturabuilderChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "Culturabuilder Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<CulturabuilderChatView>) -> NSViewController
    {
