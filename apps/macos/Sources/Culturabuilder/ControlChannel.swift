...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CulturabuilderProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "com.culturabuilder", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: CulturabuilderKit.AnyCodable]()) {
                $0[$1.key] = CulturabuilderKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or CULTURABUILDER_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: CulturabuilderProtocol.AnyCodable?) -> [String: CulturabuilderProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CulturabuilderProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CulturabuilderKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CulturabuilderProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("culturabuilder.control.heartbeat")
    static let controlAgentEvent = Notification.Name("culturabuilder.control.agent")
}
