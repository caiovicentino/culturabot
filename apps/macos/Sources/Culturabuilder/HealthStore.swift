...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "com.culturabuilder", category: "health")
    ...
    var summaryLine: String {
        ...
        if link.summary.linked != true {
            if let fallback = self.resolveFallbackChannel(snap, excluding: link.id) {
                ...
                return "\(fallbackLabel) \(fallbackState) · Not linked — run culturabuilder login"
            }
            return "Not linked — run culturabuilder login"
        }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                ...
                return "The gateway control port (\(host)) isn’t listening — restart Culturabuilder to bring it back."
            }
