...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so Culturabuilder can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = CulturabuilderLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = CulturabuilderLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(CulturabuilderLocationMode.off.rawValue)
                Text("While Using").tag(CulturabuilderLocationMode.whileUsing.rawValue)
                Text("Always").tag(CulturabuilderLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = CulturabuilderLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: CulturabuilderLocationMode {
        CulturabuilderLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: CulturabuilderLocationMode) async -> Bool {
