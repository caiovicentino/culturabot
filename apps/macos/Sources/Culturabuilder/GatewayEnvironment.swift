...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "com.culturabuilder", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["CULTURABUILDER_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = CulturabuilderConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.culturabuilderExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "culturabuilder CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["CULTURABUILDER_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "culturabuilder@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "culturabuilder@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "culturabuilder@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "culturabuilder@\(target)"])
            }

        statusHandler("Installing culturabuilder@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed culturabuilder@\(target)")
        } else {
