...
struct ConnectOptions {
    ...
    var clientId: String = "culturabuilder-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        culturabuilder-mac connect

        Usage:
          culturabuilder-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: culturabuilder-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Culturabuilder macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Culturabuilder macOS Gateway Connect")
...
private func resolvedToken(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let token = ProcessInfo.processInfo.environment["CULTURABUILDER_GATEWAY_TOKEN"], !token.isEmpty {
        return token
    }
    if mode == "remote" {
...
private func resolvedPassword(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let password = ProcessInfo.processInfo.environment["CULTURABUILDER_GATEWAY_PASSWORD"], !password.isEmpty {
        return password
    }
    if mode == "remote" {
