...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/Culturabuilder/canvas/<session>/`).
...
public let controlSocketPath = FileManager()
    ...
    .appendingPathComponent("Library/Application Support/culturabuilder/control.sock")
    .path
