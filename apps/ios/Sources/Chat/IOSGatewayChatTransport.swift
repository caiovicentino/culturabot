...
struct IOSGatewayChatTransport: CulturabuilderChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> CulturabuilderChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(CulturabuilderChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> CulturabuilderChatHistoryPayload {
        ...
        return try JSONDecoder().decode(CulturabuilderChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [CulturabuilderChatAttachmentPayload]) async throws -> CulturabuilderChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [CulturabuilderChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(CulturabuilderChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(CulturabuilderGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: CulturabuilderGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: CulturabuilderChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: CulturabuilderAgentEventPayload.self)
                        {
