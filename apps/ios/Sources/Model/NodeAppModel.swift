...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = CulturabuilderCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = CulturabuilderCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = CulturabuilderCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = CulturabuilderCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = CulturabuilderCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__culturabuilder__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: CulturabuilderLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for culturabuilder:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case CulturabuilderLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case CulturabuilderCanvasCommand.present.rawValue,
                 CulturabuilderCanvasCommand.hide.rawValue,
                 CulturabuilderCanvasCommand.navigate.rawValue,
                 CulturabuilderCanvasCommand.evalJS.rawValue,
                 CulturabuilderCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case CulturabuilderCanvasA2UICommand.reset.rawValue,
                 CulturabuilderCanvasA2UICommand.push.rawValue,
                 CulturabuilderCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case CulturabuilderCameraCommand.list.rawValue,
                 CulturabuilderCameraCommand.snap.rawValue,
                 CulturabuilderCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case CulturabuilderScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: CulturabuilderNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CulturabuilderNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(CulturabuilderLocationGetParams.self, from: req.paramsJSON)) ??
            CulturabuilderLocationGetParams()
        ...
        let payload = CulturabuilderLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CulturabuilderCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(CulturabuilderCanvasPresentParams.self, from: req.paramsJSON)) ??
                CulturabuilderCanvasPresentParams()
        ...
        case CulturabuilderCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CulturabuilderCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(CulturabuilderCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CulturabuilderCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(CulturabuilderCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CulturabuilderCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(CulturabuilderCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case CulturabuilderCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.culturabuilderA2UI) return JSON.stringify({ ok: false, error: "missing culturabuilderA2UI" });
              return JSON.stringify(globalThis.culturabuilderA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CulturabuilderCanvasA2UICommand.push.rawValue, CulturabuilderCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == CulturabuilderCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(CulturabuilderCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try CulturabuilderCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(CulturabuilderCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try CulturabuilderCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.culturabuilderA2UI) return JSON.stringify({ ok: false, error: "missing culturabuilderA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.culturabuilderA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case CulturabuilderCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CulturabuilderCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(CulturabuilderCameraSnapParams.self, from: req.paramsJSON)) ??
                CulturabuilderCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CulturabuilderCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(CulturabuilderCameraClipParams.self, from: req.paramsJSON)) ??
                CulturabuilderCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(CulturabuilderScreenRecordParams.self, from: req.paramsJSON)) ??
            CulturabuilderScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> CulturabuilderLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return CulturabuilderLocationMode(rawValue: raw) ?? .off
    }
