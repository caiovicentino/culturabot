...
@Suite struct VoiceWakePreferencesTests {
    @Test func sanitizeTriggerWordsTrimsAndDropsEmpty() {
        #expect(VoiceWakePreferences.sanitizeTriggerWords([" culturabuilder ", "", " \nclaude\t"]) == ["culturabuilder", "claude"])
    }
    ...
    @Test func displayStringUsesSanitizedWords() {
        #expect(VoiceWakePreferences.displayString(for: ["", " "]) == "culturabuilder, claude")
    }
