...
BASE_IMAGE="${BASE_IMAGE:-culturabuilder-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-culturabuilder-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=culturabuilder.sandbox=1)
NOTE
