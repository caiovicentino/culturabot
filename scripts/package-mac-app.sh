...
# Build and bundle Culturabuilder into a minimal .app we can open.
# Outputs to dist/Culturabuilder.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Culturabuilder.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Culturabuilder"
BUNDLE_ID="${BUNDLE_ID:-com.culturabuilder.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/culturabuilder/culturabuilder/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Culturabuilder/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :CulturabuilderBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :CulturabuilderGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Culturabuilder"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Culturabuilder"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Culturabuilder"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Culturabuilder" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Culturabuilder/Resources/Culturabuilder.icns" "$APP_ROOT/Contents/Resources/Culturabuilder.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Culturabuilder/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying CulturabuilderKit resources"
CULTURABUILDERKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/CulturabuilderKit_CulturabuilderKit.bundle"
if [ -d "$CULTURABUILDERKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/CulturabuilderKit_CulturabuilderKit.bundle"
  cp -R "$CULTURABUILDERKIT_BUNDLE" "$APP_ROOT/Contents/Resources/CulturabuilderKit_CulturabuilderKit.bundle"
else
  echo "WARN: CulturabuilderKit resource bundle not found at $CULTURABUILDERKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Culturabuilder"
killall -q Culturabuilder 2>/dev/null || true
