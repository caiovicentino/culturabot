...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "culturabuilder-zai-fallback-"),
  );
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "culturabuilder.json");
  ...
  const sessionId =
    process.env.CULTURABUILDER_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
