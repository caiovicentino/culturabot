...
# Culturabuilder Auth Widget for Termux
...
SERVER="${CULTURABUILDER_SERVER:-l36}"

# Check auth status
termux-toast "Checking Culturabuilder auth..."

STATUS=$(ssh "$SERVER" '$HOME/culturabuilder/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/culturabuilder/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CULTURABUILDER_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/culturabuilder/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CULTURABUILDER_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Culturabuilder won't work until re-auth"
                ;;
        esac
        ;;
