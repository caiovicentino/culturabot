#!/bin/bash
# Setup Culturabuilder Auth Management System
...
echo "=== Culturabuilder Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'culturabuilder-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Culturabuilder message: Send warning via Culturabuilder itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/culturabuilder-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/culturabuilder-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/culturabuilder-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now culturabuilder-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status culturabuilder-auth-monitor.timer"
echo ""
echo "Quick commands:"
