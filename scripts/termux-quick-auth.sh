...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/culturabuilder/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Culturabuilder Re-Auth" -c "After getting key, run: ssh $SERVER '~/culturabuilder/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
