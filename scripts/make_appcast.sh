...
ZIP=${1:?"Usage: $0 Culturabuilder-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/culturabuilder/culturabuilder/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Culturabuilder-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/culturabuilder/culturabuilder/releases/download/v${VERSION}/"}
