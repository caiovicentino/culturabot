...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "Culturabuilder")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "Culturabuilder")
...
DMG_TEMP="$(mktemp -d /tmp/culturabuilder-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/Culturabuilder/Resources/Culturabuilder.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/culturabuilder-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/culturabuilder-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/culturabuilder-dmg-limits.txt
