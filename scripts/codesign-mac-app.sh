...
APP_BUNDLE="${1:-dist/Culturabuilder.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t culturabuilder-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t culturabuilder-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t culturabuilder-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/Culturabuilder" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/Culturabuilder" "$APP_ENTITLEMENTS"
fi
