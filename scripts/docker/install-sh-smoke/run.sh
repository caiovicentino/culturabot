...
SMOKE_PREVIOUS_VERSION="${CULTURABUILDER_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CULTURABUILDER_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="culturabuilder"
if [[ -z "${CULTURABUILDER_INSTALL_PACKAGE:-}" && "$INSTALL_URL" == *"clawd.bot"* ]]; then
  DEFAULT_PACKAGE="culturabuilder"
fi
PACKAGE_NAME="${CULTURABUILDER_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
if [[ "$PACKAGE_NAME" == "culturabuilder" ]]; then
  ALT_PACKAGE_NAME="culturabuilder"
else
  ALT_PACKAGE_NAME="culturabuilder"
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CULTURABUILDER_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CULTURABUILDER_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "$CULTURABUILDER_INSTALL_LATEST_OUT"
fi
