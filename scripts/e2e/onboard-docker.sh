...
IMAGE_NAME="culturabuilder-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/culturabuilder-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/culturabuilder-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/culturabuilder-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/culturabuilder-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/culturabuilder-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.culturabuilder/culturabuilder.json"
    sessions_dir="$HOME/.culturabuilder/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.culturabuilder/culturabuilder.json" <<'"'"'JSON'"'"'
{
