...
IMAGE_NAME="culturabuilder-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/culturabuilder-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.culturabuilder/extensions"

  cat > "$HOME/.culturabuilder/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/culturabuilder-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@culturabuilder/demo-plugin-tgz",
  "version": "0.0.1",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/culturabuilder-plugin-dir.XXXXXX")"
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/culturabuilder-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@culturabuilder/demo-plugin-file",
  "version": "0.0.1",
  "culturabuilder": { "extensions": ["./index.js"] }
}
JSON
