...
IMAGE_NAME="culturabuilder-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="culturabuilder-net-e2e-$$"
GW_NAME="culturabuilder-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "CULTURABUILDER_GATEWAY_TOKEN=$TOKEN" \
	  -e "CULTURABUILDER_SKIP_CHANNELS=1" \
	  -e "CULTURABUILDER_SKIP_GMAIL_WATCHER=1" \
	  -e "CULTURABUILDER_SKIP_CRON=1" \
	  -e "CULTURABUILDER_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
