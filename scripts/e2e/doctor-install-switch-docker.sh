...
IMAGE_NAME="culturabuilder-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/culturabuilder-bin:$PATH"
  mkdir -p /tmp/culturabuilder-bin

  cat > /tmp/culturabuilder-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/culturabuilder-bin/systemctl

  cat > /tmp/culturabuilder-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/culturabuilder-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/culturabuilder"
  npm_entry="/tmp/npm-prefix/lib/node_modules/culturabuilder/culturabuilder.mjs"
  git_entry="/app/culturabuilder.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/culturabuilder-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/culturabuilder-gateway.service"
    if [ ! -f "$unit_path" ]; then
