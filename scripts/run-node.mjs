...
const compiler = env.CULTURABUILDER_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.CULTURABUILDER_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.CULTURABUILDER_RUNNER_LOG === "0") return;
  process.stderr.write(`[culturabuilder] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["culturabuilder.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
