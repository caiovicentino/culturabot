...
export const bluebubblesMessageActions: ChannelMessageActionAdapter = {
  listActions: ({ cfg }) => {
    const account = resolveBlueBubblesAccount({ cfg: cfg as CulturabuilderConfig });
    if (!account.enabled || !account.configured) return [];
    const gate = createActionGate((cfg as CulturabuilderConfig).channels?.bluebubbles?.actions);
  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    const opts = { cfg: cfg as CulturabuilderConfig, accountId: accountId ?? undefined };
