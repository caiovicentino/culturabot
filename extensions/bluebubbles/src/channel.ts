import type { ChannelAccountSnapshot, ChannelPlugin, CulturabuilderConfig } from "culturabuilder/plugin-sdk";
import {
...
export const bluebubblesPlugin: ChannelPlugin<ResolvedBlueBubblesAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listBlueBubblesAccountIds(cfg as CulturabuilderConfig),
    resolveAccount: (cfg, accountId) =>
      resolveBlueBubblesAccount({ cfg: cfg as CulturabuilderConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultBlueBubblesAccountId(cfg as CulturabuilderConfig),
    ...
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveBlueBubblesAccount({ cfg: cfg as CulturabuilderConfig, accountId }).config.allowFrom ??
        []).map(
        (entry) => String(entry),
      ),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as CulturabuilderConfig).channels?.bluebubbles?.accounts?.[resolvedAccountId],
      );
