...
function resolveCulturabuilderRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.CULTURABUILDER_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["culturabuilder"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set CULTURABUILDER_ROOT to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveCulturabuilderRoot();
