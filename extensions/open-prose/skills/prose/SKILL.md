...
metadata: {"culturabuilder":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Culturabuilder `sessions_spawn`
- **File I/O** == Culturabuilder `read`/`write`
- **Remote fetch** == Culturabuilder `web_fetch` (or `exec` with curl when POST is required)
