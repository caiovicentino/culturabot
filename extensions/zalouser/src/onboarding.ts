...
async function noteZalouserHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "",
      "Docs: https://docs.culturabuilder.com/channels/zalouser",
    ].join("\n"),
    "Zalo Personal Setup",
  );
}
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  getCurrent: (cfg) => ((cfg as CulturabuilderConfig).channels?.zalouser?.dmPolicy ?? "pairing") as "pairing",
  setPolicy: (cfg, policy) => setZalouserDmPolicy(cfg as CulturabuilderConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID
        : resolveDefaultZalouserAccountId(cfg as CulturabuilderConfig);
...
export const zalouserOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  dmPolicy,
  getStatus: async ({ cfg }) => {
    const ids = listZalouserAccountIds(cfg as CulturabuilderConfig);
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
    ...
    const defaultAccountId = resolveDefaultZalouserAccountId(cfg as CulturabuilderConfig);
    ...
    let next = cfg as CulturabuilderConfig;
