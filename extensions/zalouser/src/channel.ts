...
function resolveZalouserGroupToolPolicy(
  ...
  const account = resolveZalouserAccountSync({
    cfg: params.cfg as CulturabuilderConfig,
...
export const zalouserDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZalouserAccountSync({ cfg: cfg as CulturabuilderConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zalouserPlugin: ChannelPlugin<ResolvedZalouserAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZalouserAccountIds(cfg as CulturabuilderConfig),
    resolveAccount: (cfg, accountId) =>
      resolveZalouserAccountSync({ cfg: cfg as CulturabuilderConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZalouserAccountId(cfg as CulturabuilderConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as CulturabuilderConfig).channels?.zalouser?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    self: async ({ cfg, accountId, runtime }) => {
      ...
      const account = resolveZalouserAccountSync({ cfg: cfg as CulturabuilderConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZalouserAccountSync({ cfg: cfg as CulturabuilderConfig });
  ...
  auth: {
    login: async ({ cfg, accountId, runtime }) => {
      ...
      if (!ok) {
        throw new Error(
          "Missing dependency: `zca` not found in PATH. See docs.culturabuilder.com/channels/zalouser",
        );
      }
      runtime.log(
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      return monitorZalouserProvider({
        account,
        config: ctx.cfg as CulturabuilderConfig,
        runtime: ctx.runtime,
