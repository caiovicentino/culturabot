...
Culturabuilder extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
culturabuilder channels login --channel zalouser
...
```bash
culturabuilder message send --channel zalouser --target <threadId> --message "Hello from Culturabuilder!"
```
...
```bash
culturabuilder channels login --channel zalouser              # Login via QR
culturabuilder channels login --channel zalouser --account work
culturabuilder channels status --probe
culturabuilder channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
culturabuilder directory self --channel zalouser
culturabuilder directory peers list --channel zalouser --query "name"
culturabuilder directory groups list --channel zalouser --query "work"
culturabuilder directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
culturabuilder message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
culturabuilder message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `culturabuilder channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
culturabuilder channels login --channel zalouser --account work
culturabuilder message send --channel zalouser --account work --target <id> --message "Hello"
