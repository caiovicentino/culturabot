...
export function listMattermostAccountIds(cfg: CulturabuilderConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: CulturabuilderConfig): string {
...
export function listEnabledMattermostAccounts(cfg: CulturabuilderConfig): ResolvedMattermostAccount[] {
