...
import type { PluginRuntime } from "culturabuilder/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CULTURABUILDER_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-nostr-"));
  process.env.CULTURABUILDER_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.CULTURABUILDER_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".culturabuilder");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.CULTURABUILDER_STATE_DIR;
    else process.env.CULTURABUILDER_STATE_DIR = previous;
