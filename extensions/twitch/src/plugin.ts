/**
 * Twitch channel plugin for Culturabuilder.
 *
...
import { buildChannelConfigSchema } from "culturabuilder/plugin-sdk";
 ...
 * for Culturabuilder. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: CulturabuilderConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: CulturabuilderConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: CulturabuilderConfig): boolean => {
