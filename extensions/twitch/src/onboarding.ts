...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: CULTURABUILDER_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: CulturabuilderConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var CULTURABUILDER_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as CulturabuilderConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as CulturabuilderConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.CULTURABUILDER_TWITCH_ACCESS_TOKEN?.trim();
