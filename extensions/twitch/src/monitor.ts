...
import type { ReplyPayload, CulturabuilderConfig } from "culturabuilder/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // CulturabuilderConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as CulturabuilderConfig;
