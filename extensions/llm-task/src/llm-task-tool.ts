...
// NOTE: This extension is intended to be bundled with Culturabuilder.
// When running from source (tests/dev), Culturabuilder internals live under src/.
...
export function createLlmTaskTool(api: CulturabuilderPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "culturabuilder-llm-task-"));
