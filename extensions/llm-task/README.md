...
`clawd.invoke --each`) without adding new Culturabuilder code per workflow.
...
This extension depends on Culturabuilder internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Culturabuilder extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.culturabuilder/extensions` as a standalone plugin directory.
