...
export const zaloDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZaloAccount({ cfg: cfg as CulturabuilderConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zaloPlugin: ChannelPlugin<ResolvedZaloAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZaloAccountIds(cfg as CulturabuilderConfig),
    resolveAccount: (cfg, accountId) => resolveZaloAccount({ cfg: cfg as CulturabuilderConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZaloAccountId(cfg as CulturabuilderConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as CulturabuilderConfig).channels?.zalo?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    ...
    listPeers: async ({ cfg, accountId, query, limit }) => {
      const account = resolveZaloAccount({ cfg: cfg as CulturabuilderConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZaloAccount({ cfg: cfg as CulturabuilderConfig });
