...
async function noteZaloTokenHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Docs: https://docs.culturabuilder.com/channels/zalo",
    ].join("\n"),
    "Zalo bot token",
  );
}
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => setZaloDmPolicy(cfg as CulturabuilderConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID
        : resolveDefaultZaloAccountId(cfg as CulturabuilderConfig);
...
export const zaloOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  dmPolicy,
  getStatus: async ({ cfg }) => {
    const configured = listZaloAccountIds(cfg as CulturabuilderConfig).some((accountId) =>
      Boolean(resolveZaloAccount({ cfg: cfg as CulturabuilderConfig, accountId }).token),
    );
    return {
      channel,
      configured,
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
    ...
    const defaultZaloAccountId = resolveDefaultZaloAccountId(cfg as CulturabuilderConfig);
