...
export async function handleWhatsAppStart(host: CulturabuilderApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: CulturabuilderApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: CulturabuilderApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: CulturabuilderApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: CulturabuilderApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: CulturabuilderApp): string {
...
export function handleNostrProfileCancel(host: CulturabuilderApp) {
...
export function handleNostrProfileToggleAdvanced(host: CulturabuilderApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: CulturabuilderApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: CulturabuilderApp) {
  const state = host.nostrProfileFormState;
